...
| Destructive commands | Require `--yes` or an interactive confirmation. **Implemented 2026-08-13** (`P06-S04-M38-T01`), having been documented from the start and built nowhere. **Which commands are gated was a product-owner decision, not a derivation**: the codebase settles that exactly eight commands mutate server state, but which of those are "destructive or high-impact" is a judgement about users. The chosen line is **irreversible or costly** — `workflow cancel` (one-way terminal transition), `pack deactivate` (removes a live capability), `approve decide` (guarded against double-decide, so permanent, and governance-critical under R-001), and `experiment run` (synchronous, billable LLM calls). Deliberately **not** gated: `workflow start`, `experiment create`, `pack activate`, `config set` — the additive/creative verbs, the same line kubectl, terraform and gh draw. **One disclosed consequence:** `workflow start` triggers real agents making real LLM calls, so a money-spending path remains unprompted; that was explicit in the decision, not overlooked. A non-TTY without `--yes` is a real usage error (exit 2) naming the flag — never a prompt that would hang a script or a CI job. |
