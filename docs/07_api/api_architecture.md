...
## Implementation Status (2026-08-03, `P03-S03-M30-T06`; RFC 9457 error bodies added 2026-08-06, `P06-S01-M36-T02`; Idempotency-Key handling added 2026-08-06, `P06-S01-M36-T03`; Approvals §6.2 closed to 4/4, §6.1's `run_manifest`/`workflow_definitions`/`cancel` added, §6.4's `GET /agents` added, and §6.4's `GET /gates/results` added, 2026-08-10, `P06-S01-M36-T04`)

**Partially built — roughly 17 of the ~45 endpoints documented below exist.** Read this section before assuming an endpoint is callable.

**Built** (`kernel/src/ai_os_kernel/routes/`, all bearer-token authenticated): `POST /api/v1/workflows`, `GET /api/v1/workflows`, `GET /api/v1/workflows/{id}`, `GET /api/v1/workflows/{id}/steps`, `GET /api/v1/workflows/{id}/events`, `GET /api/v1/workflows/{id}/run_manifest`, `POST /api/v1/workflows/{id}/cancel`, `GET /api/v1/workflow_definitions`, `GET /api/v1/agents`, `GET /api/v1/gates/results`, `POST /api/v1/packs`, `POST /api/v1/packs/{id}/activate`, `POST /api/v1/packs/{id}/deactivate`, `GET /api/v1/packs/{id}`, plus the health endpoints.
...
- All Experiments, Usage, Traceability, and Logs/Traces endpoint groups — each blocked on a subsystem that is itself 0% built (Evaluation Engine, Traceability Engine). §6.4's Agents/Gates groups are each now real for one documented route (`GET /agents`, `GET /gates/results`, both 2026-08-10) — `gates/usage` and the "+ stats" half of `agents` remain unbuilt. **Approvals (§6.2) is fully real now — all 4 of its documented endpoints exist (2026-08-10).**
