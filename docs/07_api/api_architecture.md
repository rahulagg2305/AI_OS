...
## Implementation Status (2026-08-03, `P03-S03-M30-T06`; RFC 9457 error bodies added 2026-08-06, `P06-S01-M36-T02`; Idempotency-Key handling added 2026-08-06, `P06-S01-M36-T03`; Approvals §6.2 closed to 4/4, §6.1's `run_manifest`/`workflow_definitions`/`cancel` added, §6.4's `GET /agents` added, and §6.4's `GET /gates/results` added, 2026-08-10, `P06-S01-M36-T04`; **full API-vs-reality audit, 2026-08-10, `P06-S01-M36-T04`** — this section's own "Built" list and endpoint count had silently gone stale across several prior steps: §6.5 (Packs and configuration) was already fully real and 4 further real, production-wired routes existed with zero mention here at all; both corrected below; **§6.6's `traceability/impact`/`coverage` added 2026-08-11, `P04-S02-M16-T05`**; **§6.3's `POST`/`GET /experiments` + `GET /experiments/{id}` added, and `experiment:run` became a real permission, 2026-08-11, `P04-S01-M12-T12`**; **§6.3's `POST /experiments/{id}/run` added, 2026-08-11, `P04-S01-M12-T13`**; **§6.3's `GET /experiments/{id}/comparison` + `GET /experiments/{id}/runs` added, closing §6.3, 2026-08-11, `P04-S01-M12-T14`**)

**Partially built — roughly 31 of the ~45 endpoints documented below exist** (corrected 2026-08-10 from a stale "17"; +2 on 2026-08-11 for §6.6's `traceability/impact`/`coverage`, `P04-S02-M16-T05`; +3 on 2026-08-11 for §6.3's `POST /experiments`, `GET /experiments`, `GET /experiments/{id}`, `P04-S01-M12-T12`; +1 on 2026-08-11 for §6.3's `POST /experiments/{id}/run`, `P04-S01-M12-T13`; +2 on 2026-08-11 for §6.3's `GET /experiments/{id}/comparison`, `GET /experiments/{id}/runs`, `P04-S01-M12-T14`). Read this section before assuming an endpoint is callable.

**Built** (`kernel/src/ai_os_kernel/routes/`, all bearer-token authenticated): `POST /api/v1/workflows`, `GET /api/v1/workflows`, `GET /api/v1/workflows/{id}`, `GET /api/v1/workflows/{id}/steps`, `GET /api/v1/workflows/{id}/events`, `GET /api/v1/workflows/{id}/run_manifest`, `POST /api/v1/workflows/{id}/cancel`, `GET /api/v1/workflow_definitions`, `GET /api/v1/agents`, `GET /api/v1/gates/results`, `GET /api/v1/approvals`, `GET /api/v1/approvals/{id}`, `GET /api/v1/approvals/history`, `POST /api/v1/packs`, `GET /api/v1/packs`, `GET /api/v1/packs/{id}`, `POST /api/v1/packs/{id}/activate`, `POST /api/v1/packs/{id}/deactivate`, `GET /api/v1/config`, `PATCH /api/v1/config`, `GET /api/v1/config/flags`, `GET /api/v1/traceability/impact/{id}`, `GET /api/v1/traceability/coverage`, `POST /api/v1/experiments`, `GET /api/v1/experiments`, `GET /api/v1/experiments/{id}`, `POST /api/v1/experiments/{id}/run`, `GET /api/v1/experiments/{id}/comparison`, `GET /api/v1/experiments/{id}/runs`, `GET /api/v1/usage/tokens`, plus the health endpoints. **Real, but outside this document's originally-documented 45** (see "Deliberate documented divergences" below): `GET /api/v1/evaluation/cost-and-quality`, `POST`/`DELETE /api/v1/security/role-grants`, `GET /api/v1/version`, `POST /api/v1/workflows/se.delivery_pipeline`, `POST /api/v1/voice/intent`.
...
**§6.4's own `GET /api/v1/gates/results` is real too (2026-08-10), at the exact documented path — "Gate results (filterable)."** `ai_os_kernel.routes.gates.list_gate_results` reuses a new `SqlGateResultRecorder.list_all`, a plain, unguarded read over the same recorder `WorkflowInstanceService` already writes through after a real `quality_gate` step resolves (module 15's own first real Evaluation Engine data producer). Real, cursor-paginated (§9's own blanket "every collection" rule) — the keyset is `result_id` alone, a real, prefixed ULID that is already time-sortable by construction, so no separate timestamp column is needed (`gate_results` has none at all). One real, disclosed filter, `workflow_id` (backed by the already-existing `ix_gate_results_workflow_id` index); `status`/`gate_id` filtering is a real, narrower, deliberately unbuilt slice. Gated by the already-real `evaluation:read` (`permissions.py`), granted to every role — authentication_authorization.md §4.2's own `viewer` row already names "gate results" as one of that permission's intended grants, so no new permission was invented. **`GET /gates/trends` ("Pass/fail over time") remains genuinely unbuilt** — `gate_results` has no timestamp column at all, so a real trend-over-time view needs its own, separate design decision this step did not attempt.

**§6.3 Experiments is now fully real (create + read + run + compare, 2026-08-11) — `POST /api/v1/experiments` (define, `P04-S01-M12-T12`), `GET /api/v1/experiments` (list), `GET /api/v1/experiments/{id}` (detail), `POST /api/v1/experiments/{id}/run` (`T13`), `GET /api/v1/experiments/{id}/comparison`, `GET /api/v1/experiments/{id}/runs` (`T14`).** These give `evaluation.experiments` its first real writer, backed by the Kernel's own `evaluation_engine.experiment_repository.SqlExperimentRepository` — **Kernel-owned, not pack-coupled** (product-owner-delegated decision): experiment validation rules also exist in the Benchmarking pack, but the Kernel is deliberately pack-agnostic, the table + route are platform-level, and the rules (≥2 variants, ≥3 replicates) are generic statistical truths — so the platform owns its own validation and the pack's version is a legitimate mirror (the `TrustTier` precedent). Building the *create* path first was deliberate: read routes over the never-written table would have been the same "proven but idle" hollowness §6.6/the pack manifests just closed (R-018). **A real data-model mismatch was resolved with data_model.md §6 as authority:** the definition stores `variables` ("what is deliberately varied"); the per-variant `variant_key`/`model_alias` are run-time `evaluation.experiment_runs` rows, so `pinned_conditions` is empty and `status` is `defined` at definition time. `created_by` comes from the authenticated principal, never the body. Reads gated by `evaluation:read` (the same family `GET /gates/results` uses); create gated by a **new `experiment:run` permission** — §5's own table documents `experiment:read`/`experiment:run` but the code had neither as a constant, so this adds the write half (§5 names no separate "define" permission, so defining and the future launching both fall under it), granted to `operator`/`maintainer`/`admin`. **`POST /experiments/{id}/run` is now real too (2026-08-11, `P04-S01-M12-T13`)** — it is the first production caller of the already-tested `experiment_runs` writer (`SqlExperimentRunRecorder`), via the **synchronous** `evaluation_engine.experiment_run_orchestrator.ExperimentRunOrchestrator`: for each `model_alias` variant (`overview.md` §7: the model is the only legitimate variable) resolved through the real `Router`, it creates + starts + runs a real workflow per replicate and records the row (`served_from_cache=False` per §7), transitioning the experiment `defined`→`running`→`complete`. It returns **200** with a run summary (not the documented 202) because this slice runs synchronously in-request — appropriate for rare, human-defined experiments, and swappable for an async submission model later without a schema change. Gated by `experiment:run`. **Per-run model pinning is real (2026-08-11, `P04-S01-M12-T15`):** each variant's runs execute against a run-time copy of the workflow definition with every agent step's `model_alias` overridden to the variant's model (`overview.md` §5 step 5) — every real agent reads its model from `inputs['modelAlias']` (forwarded from `step.model_alias`), so the run genuinely uses the variant's model and the recorded `resolved_model_id` is what it actually used. Remaining disclosed limit: model-only `variables` (multi-factor → 422); *completing* a run against a live provider still needs a real credential (inherent to real LLM calls, not a gap). **The comparison endpoints are now real too (2026-08-11, `P04-S01-M12-T14`), completing §6.3:** `GET /experiments/{id}/comparison` returns the already-real `SqlComparisonComputer.compute()` output (mean/variance per variant × metric, cache-served runs excluded and flagged) computed live over the experiment's real `experiment_runs`/`metrics`; `GET /experiments/{id}/runs` returns the raw per-run rows via a new, small `evaluation_engine.experiment_run_reader.SqlExperimentRunReader` (nothing listed individual runs before — the computer only aggregates). Both gated `evaluation:read`; a missing experiment is 404, an existing-but-unrun one a real empty-but-valid 200.

**§6.6 Traceability — two of its five documented routes are now real (2026-08-11, `P04-S02-M16-T05`), the read half of closing the Traceability Engine's own "proven but idle" gap (risk register R-018).** `GET /api/v1/traceability/impact/{id}` (`ai_os_kernel.routes.traceability.get_impact`) and `GET /api/v1/traceability/coverage` (`get_coverage`) are thin reads over the already-real, already-tested `traceability_engine.impact_query.find_affected_artifacts`/`coverage_query.find_uncovered_requirements` — no new query logic, the identical "expose an already-real reader through a thin route" shape `GET /gates/results` established. They became worth building only once `P04-S02-M16-T04` gave the engine its first real production writer (`routes/delivery_pipeline.py` now records a real `workflow_run --produced--> documentation` link), so there is genuinely something to read — a route over a permanently-empty table would have been the same hollow shape. **One real, disclosed shape deviation on `impact/{id}`:** the underlying query identifies an artifact by its real `(artifact_type, external_id)` pair (`compute_artifact_key` hashes both), but §6.6 documents a single `{id}` segment — so `{id}` carries the `external_id` and `artifact_type` is a **required** query parameter (a hidden default would be an unsafe assumption), the same "honor the documented path, name the extra dimension explicitly" call `POST /api/v1/packs` already made. Both are unpaginated (small, bounded result sets, the `GET /approvals` reasoning) and gated by `evaluation:read` — no `traceability:read` permission is documented anywhere, and impact/coverage are read-only analytical reporting, the identical family `GET /gates/results` already reuses `EVALUATION_READ` for. **`GET /traceability/query` (the raw link graph) remains genuinely unbuilt** — its response shape is a real, separate design decision, not attempted here.

**§6.5 Packs and configuration is fully real — all 7 of its documented endpoints exist, found during this audit to have already been true for some time with no line in this section ever saying so.** `GET /api/v1/packs` (`ai_os_kernel.routes.packs.list_packs`, a plain, unfiltered read ordered by `pack_id`, no pagination — §6.5 documents none for this endpoint) closed 2026-08-06 per that route's own module docstring; `GET /api/v1/packs/{id}`/activate/deactivate/`POST /packs` (the one reasoned addition) were already real before that. `GET /api/v1/config`, `PATCH /api/v1/config`, and `GET /api/v1/config/flags` (`ai_os_kernel.routes.config`) are also real, at their exact documented paths/methods, with no shape deviation. None of this had ever been reflected in this section's own "Built" list or endpoint count.
...
- The Logs/Traces observability endpoints — no `/logs`/`/traces` route exists (no log/trace-query HTTP surface). §6.4's Agents/Gates groups are each real for one documented route (`GET /agents`, `GET /gates/results`, both 2026-08-10). **`GET /usage/tokens` is real too (2026-08-12, `P06-S01-M36-T04`), at the exact documented path/method — no shape deviation.** `ai_os_kernel.routes.usage.get_token_usage` exposes a new `evaluation_engine.token_usage_views.SqlTokenUsageViews`: token totals by model/workflow/agent/pack **including the cache split** this row names by name. That split was the gap — `evaluation.llm_calls` has recorded `cache_read_tokens`/`cache_write_tokens` on every real call since the table existed, and the pre-existing cost report carries no cache columns at all, so the data was written and never read. Gated by the already-real `evaluation:read`; unpaginated (bounded dimension sets, the `GET /approvals` reasoning); raw sums with **no invented cache hit-rate**, since no document defines the denominator and the answer changes materially with the choice. **`GET /usage/cost` is RESOLVED (2026-08-13) by amending this document, and its row has been removed from the endpoint list above** — the documented endpoint count is deliberately one lower as a result. Its capability, "cost by model/workflow/pack", is already real at `GET /api/v1/evaluation/cost-and-quality` (`P06-S03-M39-T03`, FR-095) over the identical `llm_calls` data, so a second path would have been duplicate surface, not new capability. Two alternatives were put to the product owner and declined: building a thin alias (honours the documented path, but ships two live paths for one capability — exactly the duplication the original disclosure was written to avoid), and making `/usage/cost` canonical while deprecating `/evaluation/cost-and-quality` (cleanest final naming, but the only option requiring a client change, since the Dashboard already consumes the existing route). **`GET /gates/trends` is now unblocked but still unbuilt**: its blocker was a schema decision, not effort — `evaluation.gate_results` had no timestamp column at all — and `0038_gate_results_created_at` (2026-08-13, `P04-S01-M12-T16`) added a server-defaulted, indexed `created_at`, proven bucketable by a real `date_trunc` aggregation over real rows. The route itself is deliberately not built by that step. The "+ stats" half of `agents` remains genuinely unbuilt. **§6.3 Experiments is now fully real** (`POST /experiments`, `GET /experiments`, `GET /experiments/{id}`, `POST /experiments/{id}/run`, `GET /experiments/{id}/comparison`, `GET /experiments/{id}/runs`, 2026-08-11) — every documented §6.3 endpoint exists. **§6.6 Traceability is now real for its two query-backed routes** (`GET /traceability/impact/{id}`, `GET /traceability/coverage`, 2026-08-11) — only `GET /traceability/query` (the raw link graph) and the `logs`/`traces` routes remain. **Approvals (§6.2) and Packs/configuration (§6.5) are both fully real now — every one of their documented endpoints exists.**
- **`GET /api/v1/health/detail`** — not built as its own route; a real, reasoned decision, not an oversight: `GET /health/ready`'s own `ReadinessReport` already returns the full, real per-component detail (`ai_os_kernel.health.HealthService`), so a second endpoint would only duplicate it (see `tools/aios/src/ai_os_cli/commands/health.py`'s own docstring, which reasons through the identical point for the CLI's own `health` group).
...
**Corrected by this audit**: the OpenAPI 3.1 contract and its CI snapshot test are, in fact, real — `tests/contract/test_openapi_contract.py` (`P06-S01-M36-T01`) already fails CI if the committed `docs/07_api/openapi.json` drifts from the live FastAPI app's own schema (it was built specifically because that artifact had already gone stale once, silently missing the role-administration routes). What remains genuinely unbuilt is narrower than this section previously claimed: nothing currently checks the committed `openapi.json` against *this document's own prose* (the endpoint tables and Built list above) — that is a real, disclosed gap this audit itself is evidence of, not a hypothetical one.

Deliberate documented divergences in what *is* built: `POST /api/v1/workflows` returns `200` with a reduced body rather than `202` with `definition_id`; pack activate/deactivate return `200` not `202`; `POST /api/v1/packs` is not in this document's endpoint list at all (a reasoned addition). **Four further real routes exist entirely outside this document's originally-documented 45, found by this audit and not previously disclosed here at all**: `GET /api/v1/evaluation/cost-and-quality` (`P06-S03-M39-T03`, FR-094/FR-095 — the Dashboard's own real cost/quality report, `ai_os_kernel.routes.evaluation`); `POST`/`DELETE /api/v1/security/role-grants` (`P03-S05-M14-T08` — grant/revoke an `approver:<class>` role, `ai_os_kernel.routes.role_administration`, authenticated but deliberately not flat-permission-gated, since the target service already enforces the real `admin`-only rule a flat check would only duplicate); `GET /api/v1/version` (build/version metadata, `ai_os_kernel.routes.health`); `POST /api/v1/workflows/se.delivery_pipeline` (the Software Engineering pack's own dedicated trigger for its one real production workflow, distinct from the generic `POST /workflows`). **A fifth was added deliberately on 2026-08-13: `POST /api/v1/voice/intent`** (`P06-S06-M33-T02`, `ai_os_kernel.routes.voice`) — the Voice subsystem's first production entry point, closing risk register R-018 item 5. Unlike the other four this one is an *invented* contract and is disclosed as such: neither this document nor `../14_voice_jarvis/voice_architecture.md` specifies any voice endpoint, so the path, the request/response shape and the status mapping were product-owner decisions, each reusing an existing route precedent. The body **is** the already-real `VoiceIntent` and the response the already-real `VoiceActionResult` (both snake_case, matching those models rather than introducing route-local DTOs); it is authenticated but not flat-permission-gated, because `PlatformIntentRouter` already enforces `workflow:read` for its read intents and `ApprovalService` its own class-scoped rule — the identical reasoning `POST`/`DELETE /api/v1/security/role-grants` already records. It accepts an already-structured intent, never audio. Stage B/F deliverable.
...
| `experiment:read` / `experiment:run` | Read / launch experiments. **In code (`P04-S01-M12-T12`): `experiment:run` is a real permission constant, granted to `operator`/`maintainer`/`admin`, gating `POST /experiments`; `experiment:read` is not a separate constant — experiment reads fall under `evaluation:read`, which `permissions.py` already documents as covering "experiments" (the same permission `GET /gates/results` uses).** |
...
```text
POST   /api/v1/experiments                   Define an experiment      → 201  [real, P04-S01-M12-T12]
GET    /api/v1/experiments                   List                      → 200  [real]
GET    /api/v1/experiments/{id}              Detail + run status       → 200  [real (definition; run status arrives with runs)]
POST   /api/v1/experiments/{id}/run          Launch runs               → 200  [real]
GET    /api/v1/experiments/{id}/comparison   Comparison report         → 200  [real]
GET    /api/v1/experiments/{id}/runs         Per-run detail            → 200  [real]
```
