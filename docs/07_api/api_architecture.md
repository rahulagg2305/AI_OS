...
## Implementation Status (2026-08-03, `P03-S03-M30-T06`; RFC 9457 error bodies added 2026-08-06, `P06-S01-M36-T02`; Idempotency-Key handling added 2026-08-06, `P06-S01-M36-T03`; Approvals §6.2 closed to 4/4, §6.1's `run_manifest`/`workflow_definitions`/`cancel` added, §6.4's `GET /agents` added, and §6.4's `GET /gates/results` added, 2026-08-10, `P06-S01-M36-T04`; **full API-vs-reality audit, 2026-08-10, `P06-S01-M36-T04`** — this section's own "Built" list and endpoint count had silently gone stale across several prior steps: §6.5 (Packs and configuration) was already fully real and 4 further real, production-wired routes existed with zero mention here at all; both corrected below)

**Partially built — roughly 23 of the ~45 endpoints documented below exist** (corrected 2026-08-10 from a stale "17" — a full, direct audit against the freshly-regenerated `openapi.json`, endpoint by endpoint, found the running count had not kept pace with §6.2/§6.5 closing out and §6.4 gaining two routes across several prior steps). Read this section before assuming an endpoint is callable.

**Built** (`kernel/src/ai_os_kernel/routes/`, all bearer-token authenticated): `POST /api/v1/workflows`, `GET /api/v1/workflows`, `GET /api/v1/workflows/{id}`, `GET /api/v1/workflows/{id}/steps`, `GET /api/v1/workflows/{id}/events`, `GET /api/v1/workflows/{id}/run_manifest`, `POST /api/v1/workflows/{id}/cancel`, `GET /api/v1/workflow_definitions`, `GET /api/v1/agents`, `GET /api/v1/gates/results`, `GET /api/v1/approvals`, `GET /api/v1/approvals/{id}`, `GET /api/v1/approvals/history`, `POST /api/v1/packs`, `GET /api/v1/packs`, `GET /api/v1/packs/{id}`, `POST /api/v1/packs/{id}/activate`, `POST /api/v1/packs/{id}/deactivate`, `GET /api/v1/config`, `PATCH /api/v1/config`, `GET /api/v1/config/flags`, plus the health endpoints. **Real, but outside this document's originally-documented 45** (see "Deliberate documented divergences" below): `GET /api/v1/evaluation/cost-and-quality`, `POST`/`DELETE /api/v1/security/role-grants`, `GET /api/v1/version`, `POST /api/v1/workflows/se.delivery_pipeline`.
...
- All Experiments and Traceability/Logs/Traces endpoint groups — each blocked on a subsystem that is itself 0% built at this API surface (no `/experiments`/`/traceability`/`/logs`/`/traces` route exists; the Benchmarking Pack and Traceability Engine have real internals but no HTTP exposure yet). §6.4's Agents/Gates groups are each real for one documented route (`GET /agents`, `GET /gates/results`, both 2026-08-10) — `usage/cost`, `usage/tokens`, `gates/trends`, and the "+ stats" half of `agents` remain unbuilt. **Approvals (§6.2) and Packs/configuration (§6.5) are both fully real now — every one of their documented endpoints exists.**
