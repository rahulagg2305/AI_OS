...
- The Logs/Traces observability endpoints — no `/logs`/`/traces` route exists (no log/trace-query HTTP surface). §6.4's Agents/Gates groups are each real for one documented route (`GET /agents`, `GET /gates/results`, both 2026-08-10). **`GET /usage/tokens` is real too (2026-08-12, `P06-S01-M36-T04`), at the exact documented path/method — no shape deviation.** `ai_os_kernel.routes.usage.get_token_usage` exposes a new `evaluation_engine.token_usage_views.SqlTokenUsageViews`: token totals by model/workflow/agent/pack **including the cache split** this row names by name. That split was the gap — `evaluation.llm_calls` has recorded `cache_read_tokens`/`cache_write_tokens` on every real call since the table existed, and the pre-existing cost report carries no cache columns at all, so the data was written and never read. Gated by the already-real `evaluation:read`; unpaginated (bounded dimension sets, the `GET /approvals` reasoning); raw sums with **no invented cache hit-rate**, since no document defines the denominator and the answer changes materially with the choice. **`GET /usage/cost` is deliberately NOT built, and this is a disclosed divergence rather than remaining work**: its capability — "cost by model/workflow/pack" — is already real at `GET /api/v1/evaluation/cost-and-quality` (`P06-S03-M39-T03`, FR-095) over the identical `llm_calls` data, so a second path would be duplicate surface, not new capability. Whether to alias the documented path or amend this document is a real product-owner decision, recorded here rather than resolved unilaterally. `gates/trends` and the "+ stats" half of `agents` remain genuinely unbuilt. **§6.3 Experiments is now fully real** (`POST /experiments`, `GET /experiments`, `GET /experiments/{id}`, `POST /experiments/{id}/run`, `GET /experiments/{id}/comparison`, `GET /experiments/{id}/runs`, 2026-08-11) — every documented §6.3 endpoint exists. **§6.6 Traceability is now real for its two query-backed routes** (`GET /traceability/impact/{id}`, `GET /traceability/coverage`, 2026-08-11) — only `GET /traceability/query` (the raw link graph) and the `logs`/`traces` routes remain. **Approvals (§6.2) and Packs/configuration (§6.5) are both fully real now — every one of their documented endpoints exists.**
