...
**A real HTTP entry point exists as of 2026-08-13 (`P06-S06-M33-T02`): `POST /api/v1/voice/intent`.** Until then this whole subsystem was real, tested and *unreachable* — zero production importers, no route, no entry point (risk register R-018 item 5). `ai_os_kernel.routes.voice` is a thin route that assembles the already-real `PlatformIntentRouter` from already-bootstrapped collaborators and calls its already-real `handle`; no intent logic moved. **This document specifies no voice endpoint, and neither does `../07_api/api_architecture.md`, so the contract was invented there and is disclosed as such**: the path follows the `/api/v1/<subsystem>/<noun>` shape existing routes use, the request body **is** `VoiceIntent` and the response **is** `VoiceActionResult` (no parallel DTOs that could drift), and the route is authenticated but not flat-permission-gated because the router already applies the real gates. Proven through the real `build_app()` composition against real Postgres. **It accepts an already-structured intent, never audio** — see the paragraph below, which is unchanged.
