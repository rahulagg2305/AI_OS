...
**Updated 2026-08-09 (`P08-S02-M30-T01`): `StepType.FOREACH` and a real executor now exist — one of this decision's two named-but-undesigned step types is closed, `compensate` remains the other.** `WorkflowStep` gained the matching `ForeachSpec` contract (`sourceStepId`/`itemsField`/`maxFanOut`, see `workflow_architecture.md`'s new "Foreach Step Contract"); `ForeachStepExecutor` reads a named prior step's own real, persisted list output, refuses the whole step before creating any child if the real item count exceeds the declared `maxFanOut` (point 3 above: "no unbounded agent-driven loop"), and otherwise creates, starts, and runs one real, separate child `WorkflowInstance` per item — reusing `SubWorkflowStepExecutor`'s own per-item mechanism sequentially, not concurrently (a real, disclosed, narrower scope than `ParallelStepExecutor`; this decision names `foreach` as a bounded fan-out over a plan artifact, not a concurrency primitive). Proven end to end against a real Postgres container: a two-item plan artifact fans out into two real, separate, completed children, and an over-bound item count is genuinely refused before any child is created. **The plan-artifact-plus-`foreach` pattern is still not chained into a real, running workflow, for a newly-discovered, different reason than before**: wiring `foreach` into `se.product_creation`'s own step 8 (`sourceStepId: technical-planner`) surfaced that `technical-planner`'s own real plan-item shape (`taskId`/`title`/`description`, `P03-S02-M29-T08`) does not satisfy `se.implement_task`'s own real `inputs` schema (`{task: string}`, `additionalProperties: false`, `P08-S02-M30-T02`) — two independently-built, already-`done` tickets whose contracts were never checked against each other until a real `foreach` step tried to connect them. `foreach_executor` remains unwired into any real composition — the identical `parallel_executor`/`sub_workflow_executor` precedent — pending that resolution. See `P08-S02-M30-T01`'s own ticket for the two real options this leaves open.
