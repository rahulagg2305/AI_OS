...
**Update (2026-08-07, `P07-S02-M14-T02`): "Full five-role model" — closing a real, remaining gap, and correcting the line above's stale "none of the five roles exist as such" claim.** All 5 documented roles (`viewer`/`operator`/`approver`/`maintainer`/`admin`) were already correctly modelled and enforced via bearer-token-claimed roles (`security_manager.permissions._ROLE_PERMISSIONS`, checked against every real, permission-checked route) — that claim no longer held even before this ticket. The real, remaining gap: a persisted role grant (`RoleAdministrationService.grant()`, real since `P03-S05-M14-T07`) only ever took effect inside `ApprovalService.decide()`'s own narrow check, never for a general route. The union logic is now a shared `security_manager.role_administration.resolve_effective_roles()`, reused unchanged by `decide()` and newly by the general `authenticate()` FastAPI dependency, reading an optional, bootstrap-wired `role_grant_repository`. Proven end to end, real Postgres, real HTTP: a bearer token with no roles at all is refused `GET /api/v1/packs` (`pack:read`, granted only to `maintainer`/`admin`), a real, persisted `maintainer` grant for that identical, unchanged token then genuinely enables the identical request, and a real revoke disables it again. The workflow term and the full four-way single-call intersection still do not exist.
