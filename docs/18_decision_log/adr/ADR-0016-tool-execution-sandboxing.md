...
**Related finding, 2026-08-10 (`P07-S01-M40-T01`, a real gVisor feasibility investigation for the *Kubernetes* sandbox pattern, deployment_architecture.md §6): `runsc` genuinely runs on this project's own dev-machine kernel (Docker Desktop/WSL2) — a throwaway `kind` node running a real `runsc`-backed pod showed the genuine gVisor kernel signature in its logs.** This is real, positive evidence that this ADR's own "gated on a host that has it installed" caveat is closeable in principle on this exact hardware, but it does **not** itself close the gap recorded above: that investigation registered `runsc` inside a disposable `kind` node's own containerd config, not this machine's actual local Docker Engine (`docker_executor.py`'s own target). Doing the latter needs a persistent edit to Docker Desktop's `daemon.json` — a host change outside this repo, deliberately not made without explicit product-owner approval (see `P03-S01-M20-T05`'s own ticket for the same finding recorded from this ADR's own governed ticket).
