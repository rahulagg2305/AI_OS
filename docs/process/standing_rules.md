...
- **Run the *complete* local suite before pushing, never only the touched-area subset (added 2026-08-13, after this exact mistake cost a failed CI round-trip).** The failing step had run `tests/integration/workflow_engine` and `tests/integration/event_bus` — genuinely the affected areas — and CI then failed on two things neither could ever have caught: `tests/integration/persistence`'s schema-parity guard, which pins `evaluation.gate_results`' real columns against the documented list and had not been told about a new migration; and the Dashboard job's OpenAPI client drift check, which is not a pytest suite at all. **The lesson is not "run more tests" but "your judgement about which areas are affected is exactly the thing that is unreliable"** — a schema change reaches a documentation-parity test in a package you never opened, and an added route reaches a generated TypeScript client. Concretely, before any push: the full `tests/unit tests/contract tests/roadmap`, the full `tests/integration`, and — whenever `docs/07_api/openapi.json` changes — `npm run check:api-client` plus the dashboard tests. The full integration run is ~20 minutes; a failed CI round-trip plus the re-verification costs more.
