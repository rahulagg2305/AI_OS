...
**Built:** `structlog` JSON logging with trace-context binding; a real OpenTelemetry `TracerProvider` producing a genuine span per HTTP request with the documented attributes; **exactly one** metric, `aios.http.requests`. **Real OTLP/HTTP export exists** (`P01-S05-M04-T03`) — `configure_tracing()`/`configure_metrics()` use a real `OTLPSpanExporter`/`OTLPMetricExporter` when `OTEL_EXPORTER_OTLP_ENDPOINT` is set (a bootstrap env var, `ObservabilitySettings`), proven against a real receiver in tests; the console exporters remain the default. **The Collector and reference backends are real too** (`P01-S05-M04-T04`): `infra/docker-compose.yml`'s `observability` profile — real Prometheus, Tempo, Loki, and a provisioned Grafana with one real dashboard panel (`aios.http.requests`), proven end to end against a real `docker compose up`. **Grafana declares a real healthcheck as of 2026-08-12**, so `docker compose up --wait` genuinely gates on its readiness rather than returning as soon as the container is running: Docker publishes the port immediately, but Grafana runs database migrations before serving, leaving a measured ~12-second window in which a connection is accepted and then reset. That window failed a real CI run; see the risk register for the full root cause. **Extended to Prometheus, Tempo and Loki the following day** (`P01-S05-M04-T07`, 2026-08-13), each against its own documented readiness endpoint (`/-/healthy`, `/ready`, `/ready`), so `docker compose up --wait` now gates on the whole profile rather than one service. Each probe command was verified against its own image rather than copied between them — Loki ships busybox at `/busybox/wget`, not on `PATH`, so a copied command would have marked it permanently unhealthy. **`otel-collector` is the one service that genuinely cannot have one**: its image is distroless, with no shell and no `wget`/`curl`/`nc`, so neither a `CMD-SHELL` nor an exec-form probe can run inside it — the Collector's own `health_check` extension would expose an endpoint that nothing in the container could call. Callers needing its readiness must poll from outside, as the compose-profile test does. The audit path (`governance.audit_log`) is real too (`P01-S05-M04-T05`/`T06`): a hash-chained writer, `row_hash`/`prev_hash` computed per row, and a scheduled verification job started in `_lifespan`.
