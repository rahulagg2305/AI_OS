...
**Still genuinely partial — a disclosed, product-owner-approved scope, not the full framework.** RBAC (`security_architecture.md` §9: "approval classes are granted separately") is now enforced, and role *administration* (granting a real principal an `approver:<class>` role in the first place) is now real too (see above); no notification channel of any kind (the Notification Service is still 0% built); no automatic `timed_out`/`cancelled` transition or escalation policy (no reaper job exists yet, mirroring `WorkflowLeaseReaper`'s own earlier, separate step); `changes_requested`'s own "loop back to an earlier step" semantics are undefined anywhere in this document and are not built. **Found while investigating `P06-S03-M39-T02` (the Dashboard's own Pending Approvals view): there is no way to list approvals at all.** `ApprovalRepository`'s real interface (`get_by_step`/`get_by_id`/`create_pending`/`decide`) has no query returning more than one row, and `routes/approvals.py` has exactly one real route (deciding a specific, already-known approval by id) — a caller must already know a `(workflow_id, step_id)` or `approval_id` to read anything; there is no "what is pending right now" query at either the repository or HTTP layer. See `ai_os_kernel.workflow_engine.human_approval`'s own module docstring for the full reasoning.
