...
## Implementation Status (2026-07-28; embedding writer added 2026-08-04; vector search added 2026-08-04; RRF fusion added 2026-08-04; retrieval service added 2026-08-04; indexing component added 2026-08-04)
...
**The Indexing Pipeline box is now real too**, as of `P02-S04-M09-T03`: `ai_os_kernel.knowledge_manager.indexing.IndexingService` — real, deterministic, versioned chunking (`chunk_strategy_version="fixed-size-v1"`, fixed-size character windows with overlap) through the real, unchanged `SqlKnowledgeWriter` — matching this line's own "deterministic, versioned `chunk_strategy_version`" language exactly. It also adds a real archive-and-replace change policy (§6's "same query returns consistent results" needs superseded content to stop being current): re-indexing an unchanged `source_uri`/content hash is a no-op; changed content archives the prior `knowledge.documents` row (`archived_at`) and writes a fresh one. **Disclosed, tested limitation:** neither real searcher below filters on `archived_at` yet, so a superseded chunk can still surface in a real search result — proven directly, not just asserted, in `test_indexing.py`.

**Not built:** **the documented HNSW index** — §4 names "pgvector HNSW, cosine distance," but HNSW requires a fixed vector dimension to create at all, and no real embedding model/dimension has been chosen as authoritative yet (the same already-disclosed ambiguity `persistence/knowledge_schema.py` names for the column itself). The exact `<=>` query above answers the identical nearest-neighbour question correctly without it; adding the index later, once a dimension is real, is a purely additive migration. `RetrievalService` does not call `embed()` — `RetrievalRequest.query_vector` is caller-supplied, a disclosed scope boundary (the task named exactly 3 pieces to compose, not the LLM Gateway too). `IndexingService` produces no embeddings either — nothing yet calls the real embedding writer from a real ingestion path. Also absent: Metadata Filter, SQL-predicate Access Control, and archived-row filtering in either real searcher (see above). **No consumer calls any of this** — no Context Manager resolver, and no API route calls either.
