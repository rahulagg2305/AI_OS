...
**Not built:** **the documented HNSW index** — §4 names "pgvector HNSW, cosine distance," but HNSW requires a fixed vector dimension to create at all, and no real embedding model/dimension has been chosen as authoritative yet (the same already-disclosed ambiguity `persistence/knowledge_schema.py` names for the column itself). The exact `<=>` query above answers the identical nearest-neighbour question correctly without it; adding the index later, once a dimension is real, is a purely additive migration. `RetrievalService` does not call `embed()` — `RetrievalRequest.query_vector` is caller-supplied, a disclosed scope boundary (the task named exactly 3 pieces to compose, not the LLM Gateway too); `QueryEngine` keeps that identical boundary. `IndexingService` produces no embeddings either — nothing yet calls the real embedding writer from a real ingestion path. Also absent: Metadata Filter, SQL-predicate Access Control, and archived-row filtering in either real searcher directly (see above). **No consumer calls any of this** — no Context Manager resolver, and no API route calls either.
