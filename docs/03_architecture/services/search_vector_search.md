...
## Implementation Status (2026-07-28; embedding writer added 2026-08-04; vector search added 2026-08-04)

**Partially built — keyword search, a real embedding writer, and now real vector search; still no Hybrid Ranker.**

**Built:** the four `knowledge.*` tables (migration `0029`, with the `pgvector` extension enabled and a generated `content_tsv` column + GIN index); a real writer (`kernel/src/ai_os_kernel/persistence/knowledge_writer.py`) persisting a document and its already-chunked content in one transaction; a real keyword searcher (`knowledge_keyword_search.py`) ranking chunks via `plainto_tsquery`/`ts_rank`; a real embeddings writer (`kernel/src/ai_os_kernel/retrieval/embedding_writer.py`: `SqlEmbeddingWriter` + `embed_chunk`, `P02-S04-M11-T03`) that calls the real, already-built `embed()` (`P02-S02-M06-T09`) and persists a genuine vector into `knowledge.embeddings`; and, as of `P02-S04-M11-T04`, real vector search (`retrieval/vector_search.py`: `SqlVectorSearcher`) using pgvector's real `<=>` cosine-distance operator, `ORDER BY`/`LIMIT` — an exact, not approximate, nearest-neighbour ranking. Every query is structurally filtered by `embedding_model_id`/`embedding_model_version` (§4's "queries compare only vectors from the same model and version"); `index_generation` is an optional pin, matching §4's own "pinnable in a request" language. Proven end to end against real Postgres/pgvector: hand-computed cosine distances come back in the mathematically correct order, a closer vector from a different embedding model is genuinely excluded, and a real generation pin genuinely narrows the result set.

**Not built:** **the documented HNSW index** — §4 names "pgvector HNSW, cosine distance," but HNSW requires a fixed vector dimension to create at all, and no real embedding model/dimension has been chosen as authoritative yet (the same already-disclosed ambiguity `persistence/knowledge_schema.py` names for the column itself). The exact `<=>` query above answers the identical nearest-neighbour question correctly without it; adding the index later, once a dimension is real, is a purely additive migration. **The Hybrid Ranker and Reciprocal Rank Fusion** — neither exists; keyword and vector search are two separate, real, unconnected paths today. Also absent: the Indexing Pipeline and chunking engine (both writers require input already chunked/hashed precisely because nothing produces it), metadata/`trust`/`project_id` filtering, SQL-predicate access control, and a Retrieval Service beyond these two real pieces. **No consumer calls any of this** — no Context Manager resolver, no Knowledge Manager, and no API route calls either.
