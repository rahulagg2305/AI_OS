...
## Implementation Status (2026-07-28; embedding writer added 2026-08-04; vector search added 2026-08-04; RRF fusion added 2026-08-04; retrieval service added 2026-08-04; indexing component added 2026-08-04; query engine added 2026-08-04)
...
**The Indexing Pipeline box is now real too**, as of `P02-S04-M09-T03`: `ai_os_kernel.knowledge_manager.indexing.IndexingService` — real, deterministic, versioned chunking (`chunk_strategy_version="fixed-size-v1"`, fixed-size character windows with overlap) through the real, unchanged `SqlKnowledgeWriter` — matching this line's own "deterministic, versioned `chunk_strategy_version`" language exactly. It also adds a real archive-and-replace change policy (§6's "same query returns consistent results" needs superseded content to stop being current): re-indexing an unchanged `source_uri`/content hash is a no-op; changed content archives the prior `knowledge.documents` row (`archived_at`) and writes a fresh one. **Disclosed limitation, now partially closed:** neither real searcher below filters on `archived_at` itself. As of `P02-S04-M09-T04`, `ai_os_kernel.knowledge_manager.query_engine.QueryEngine` — sitting above `RetrievalService` to add real provenance (source `document_id`/`source_uri`/`trust`/`content` per result, satisfying this doc's own §3 "return ranked results with provenance") — joins back to `knowledge.documents` and filters `archived_at IS NULL` in that same join, so a caller of `QueryEngine` genuinely never sees a superseded chunk. A caller reaching `RetrievalService` or either real searcher directly still can — proven directly in both directions in `test_query_engine.py`, not just asserted. **As of `P02-S04-M09-T05`**, that same join also attaches real *version* provenance — every result's own `chunk_strategy_version` (line 86's own requirement, always present) plus `embedding_model_id`/`embedding_model_version`/`index_generation` when a real embedding row exists under the request's exact queried model/version (a genuine, disclosed `None` otherwise, never fabricated) — satisfying line 94's "indexing must preserve provenance (source document / ID / version)" for every result, not only the ones with a real vector match.

**Not built:** **the documented HNSW index** — §4 names "pgvector HNSW, cosine distance," but HNSW requires a fixed vector dimension to create at all, and no real embedding model/dimension has been chosen as authoritative yet (the same already-disclosed ambiguity `persistence/knowledge_schema.py` names for the column itself). The exact `<=>` query above answers the identical nearest-neighbour question correctly without it; adding the index later, once a dimension is real, is a purely additive migration. `RetrievalService` does not call `embed()` — `RetrievalRequest.query_vector` is caller-supplied, a disclosed scope boundary (the task named exactly 3 pieces to compose, not the LLM Gateway too); `QueryEngine` keeps that identical boundary. **Updated 2026-08-14 (`P02-S04-M09-T06`): `IndexingService` now calls the real embedding writer from the real ingestion path**, once per freshly-written chunk, when its optional `embedder`/`embedding_writer`/`embedding_model_alias` are supplied together — so §4's "Indexing Pipeline" box genuinely produces vectors and freshly-indexed content is immediately vector-searchable (proven by a real `SqlVectorSearcher` query over real pgvector). Embedding stays opt-in because it is billable network work (product-owner decision), and §5's "models used for embedding must be configuration-driven" is honoured by taking a model *alias*, never a literal model id (ADR-0002). **Disclosed against ADR-0013:** chunks and vectors commit in sequential transactions rather than one, because `SqlEmbeddingWriter` owns its own and the alternative holds a transaction open across N provider calls — the ADR's intent, not its literal letter. No production caller enables it yet (`P02-S04-M09-T07`). Also absent: Metadata Filter, SQL-predicate Access Control, and archived-row filtering in either real searcher directly (see above). **No consumer calls any of this** — no Context Manager resolver, and no API route calls either.
