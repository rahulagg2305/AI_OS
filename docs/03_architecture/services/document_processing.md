...
**Two of the five documented parser adapters are not built, disclosed rather than silently assumed:** PDF and DOCX/Office both need a real, new third-party dependency (`pypdf`, `python-docx` or equivalent) this project does not yet declare — a deliberate scope decision (design fork resolved via `AskUserQuestion`), not an oversight. `detect_format()` raises a clear, typed `UnsupportedDocumentFormatError` naming PDF/DOCX specifically as deferred, rather than mis-detecting or crashing on them. Also not built: the Structure Extractor/Metadata Enricher/Output Normalizer/Observability Hook boxes beyond what each parser's own `metadata` dict already reports, and the Chunking Engine — `P05-S01-M26-T02` (Chunking pipeline) is its own, already-declared, dependent ticket. The one adjacent real component, `kernel/src/ai_os_kernel/persistence/knowledge_writer.py`, still accepts only **already-chunked** input, since no chunking pipeline exists yet to produce it. Its main intended consumer, the Project Intelligence pack, is also 0% built.
