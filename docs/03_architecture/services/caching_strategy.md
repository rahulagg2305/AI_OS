...
**Updated 2026-08-13 (`P02-S07-M23-T03`): the `Cache` Protocol is real, and so is a second implementation.** `caching/cache.py` defines `Cache` (`get`/`set`/`delete` — the first two from what `ResponseCache` genuinely does against Redis, `delete` from §2's own explicit "support explicit invalidation" requirement), plus `RedisCache` (the production adapter) and `InMemoryCache` (the real fake ADR-0004 asks for, with TTL genuinely honoured on a monotonic clock). `ResponseCache` now takes the Protocol rather than a `Redis`, so it is exercisable with no container at all — proven by a real round trip in a plain unit test. An adapter was needed rather than structural typing because mypy confirmed `Redis` does **not** satisfy the Protocol (`name`/`ex` vs `key`/`ttl_seconds`). **Not built:** no platform cache (resolved configuration, resolved secrets, parsed documents, retrieval results, pack registry metadata); no rate limiter; `ResponseCache` is not wired into `llm_gateway.gateway`'s real call path yet — nothing in a real Kernel composition calls it. Stage B deliverable, still in progress.
