...
**As-built reconciliation (2026-08-11, `P04-S01-M12-T12`) — read this before treating the "Define an experiment | Benchmarking Pack" row above as the whole story.** Building §6.3's `POST /api/v1/experiments` surfaced a real, pre-existing conflict between two approved documents: this section assigns experiment *definition* to the Benchmarking Pack, but `api_architecture.md §6.3` documents `POST /api/v1/experiments` as a **platform** HTTP endpoint — and the Kernel is deliberately **pack-agnostic** (it hard-codes no pack knowledge), so a platform route cannot depend on a specific pack to validate. All three cannot hold cleanly. The product owner delegated the resolution; it was made toward the platform: the **Kernel now owns a real platform experiment definition** — validation (`>= 2` variants, `>= 3` replicates) and persistence (`evaluation.experiments`) live in `evaluation_engine.experiment_repository.SqlExperimentRepository`, exposed over `POST`/`GET /api/v1/experiments`. The rules are generic statistical truths, not Benchmarking-specific, so the Benchmarking Pack's own `ExperimentSpec`/`validate_experiment_spec` is now best read as a **legitimate independent mirror** across the Kernel/pack boundary (the `TrustTier` precedent), richer than the platform's (it also carries an explicit `variants` list and a `cost_ceiling_usd`) for pack-side/programmatic definition. The table row above is therefore refined, not overturned: the *pack* may still define experiments its own way; the *platform* also defines them, as the documented API requires. **Reconciling the two into one shared source of truth — or formally re-deciding this boundary — is real, separate follow-up work a product owner should weigh in on;** this note records the as-built reality so it is not silently divergent.
