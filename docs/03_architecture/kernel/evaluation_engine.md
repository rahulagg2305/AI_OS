...
**Not built:** the rest of §5 — no Metrics Aggregator, no Comparison Computer, no Results Store writer, no Reporting Interface (the `gate_results`/`run_manifests`/`metrics` writers above are narrow, direct persistence writes, not any of those designed components — and `metrics` has no real production caller yet, see "Built" above). Consequently **most of §6's mandatory experiment guarantees remain unenforced**: the run manifest is now written for every real completed run (see above, with its 4 disclosed gaps), but there is still no per-replicate storage, no mean/variance computation, no `served_from_cache` exclusion, no prompt-adaptation recording. The Benchmarking Pack that §5.1 assigns experiment *definition* to does not exist either (`capability_packs/` contains only `software-engineering/` and a `_template/`), so neither side of that boundary is built — this is also exactly why the real Metrics Collector above has no real production caller. §3's metric categories have no *other* real, additional collector beyond the ones just named: the only other real metric in the entire Kernel is `aios.http.requests` (`kernel/src/ai_os_kernel/observability/metrics.py`). Roadmap stage: **D**.
