...
| **Health check protocols** | **Decided and built (2026-07-30).** The return shape is `HealthReport` (`pack_contract.py`); the caller and cadence policy — "who calls a pack's health check and how often" — is now `ai_os_kernel.capability_manager.health_poller.poll_pack_health()`, the smallest real slice: `POLL_INTERVAL_SECONDS`=30.0, `POLL_TIMEOUT_SECONDS`=5.0, `CONSECUTIVE_FAILURE_THRESHOLD`=3 (Kubernetes' own common `failureThreshold` default), all named constants. Genuinely resolves every one of a pack's own `catalog.agents` rows through a real `AgentRegistry`, writes a real `catalog.packs.health` snapshot every poll (`record_health()`), and calls the new `mark_failed()` once the threshold is crossed. **Named remaining gap:** `POLL_INTERVAL_SECONDS` is decided but not yet enforced — there is no background scheduler; the one real caller (`ai_os_kernel.bootstrap._lifespan`) runs exactly one poll per pack per Kernel startup. A future step adds a real periodic scheduler that reads this same constant, not a new one. |
