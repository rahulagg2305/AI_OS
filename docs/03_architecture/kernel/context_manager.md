...
| **Ranking / filtering strategy** | **Cross-source ranking is now real (`P02-S03-M08-T09`), but only as a raw score comparison — whether that is the *right* long-term policy stays genuinely open.** [ADR-0013](../../18_decision_log/adr/ADR-0013-search-and-vector-store.md) already decides how *retrieval* ranks — keyword (Postgres FTS) plus vector (pgvector) combined by Reciprocal Rank Fusion — so ranking *within one retrieval source* was never open. `manager.py`'s `_rank_by_relevance` now genuinely sorts every candidate, from every source, by `relevance_score` directly, with no per-source weighting — real, tested, and honestly the simplest thing that could work with exactly two real sources today. What remains open: this raw comparison applies **no authority weighting** at all, so the documented hard constraint — Knowledge outranks Memory in *authority* (`../../20_glossary/glossary.md` §3, `memory_manager.md` §6) — is not yet enforced as a rule, only true by coincidence of today's two sources' own score scales. Settling whether raw-score comparison remains correct, or needs an authority-aware weighting layered on top, needs a real Memory resolver to calibrate against — deliberately left open until one exists, the same reasoning `memory_manager.md` §6.1 applies to promotion heuristics. §7's determinism rule is satisfied either way: `_rank_by_relevance` is a stable sort, so ties never break arbitrarily. |
