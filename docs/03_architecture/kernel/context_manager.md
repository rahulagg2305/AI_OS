...
**Not built:** **all 6 of the 6 sources in §3 are now real** (Workflow State, Knowledge, Memory, Runtime Configuration, AI Context Packs) plus User-provided inputs folded into `WorkflowStateResolver` — every documented source now has a real resolver. `memory_manager/` itself remains a docstring-only `__init__.py` — `MemoryResolver` calls a real store one layer down (`persistence/memory_writer.py`), the identical relationship `KnowledgeResolver` has to `knowledge_manager/query_engine.py`. No automatic, `applies_to`-based AI Context Pack selection exists — `pack_references` must be caller-supplied. `AIContextPackResolver` is still not wired into any production composition, and `P02-S03-M08-T14` found this genuinely, correctly blocked, not merely undone (`RuntimeConfigResolver`/`KnowledgeResolver`/`MemoryResolver` are wired, as of `P02-S03-M08-T11`/`T12`/`T13` — see above): `ai_context/` has zero real, authored packs (`ai_context_strategy.md`'s own Implementation Status: "Built: nothing"), so any real wiring would need either a fabricated `pack_references=()` (contributing nothing, presented as real production value — explicitly out of scope) or a genuinely new, curated pack, which is a content-authoring decision (§5: "curated over complete"), not an engineering one this resolver-wiring ticket should invent. The Filter/Ranker does not filter by anything beyond relevance/budget — no minimum-score threshold, no `required_context_types` selection (that field still does not exist on `ContextRequest`), since nothing documents a real criterion to filter by yet. `index_generation` is specified in §6 but `KnowledgeResolver` does not pin one (`RetrievalRequest.index_generation` defaults to unpinned) — no producer creates a second real generation yet either. §6's `trust` field is enforced as a required field; `KnowledgeResolver` is the first source to emit a genuinely variable (not fixed-constant) `trust` per item, reflecting the queried document's own real classification. Token counting is a character-length estimate (`_CHARS_PER_TOKEN_ESTIMATE = 4`), not a provider token count — the LLM Gateway has no `count_tokens()` wired in here yet (it exists, `P02-S02-M06-T10`, but this package does not call it).

Authoritative, always-current status: `../../19_roadmap/feature_inventory.md` (per-module completion table). Detailed build history: `../../19_roadmap/history/INDEX.md` (specifically `012_context_manager.md`).
...
**The "Knowledge Resolver" bullet above is now real too (2026-08-04, `P02-S03-M08-T05`).** `ai_os_kernel.context_manager.resolvers.KnowledgeResolver` calls the real Knowledge Manager `QueryEngine` (§7's own related component, `knowledge_manager.md`) — the first genuine consumer any Knowledge Manager/Retrieval component built this session has had. Proven end to end against real Postgres and a real local embeddings server: a real document indexed through `IndexingService`, queried by real text, lands in a real `AssembledContext` alongside a real `WorkflowStateResolver` item, with real provenance and a real, non-constant `relevance_score`. Wired into a real production composition later, as of `P02-S03-M08-T12` — the identical "real, proven, not yet wired" precedent `sub_workflow`/`parallel`/`decision` step types established in the Workflow Engine before their own first real callers, then closed the same way.

**`KnowledgeResolver` reaches a real production composition too (2026-08-05, `P02-S03-M08-T12`), into `se.delivery_pipeline`'s `requirements-analyst` step.** `ai_os_kernel.workflow_engine.delivery_pipeline._QueryFromRequirementInputResolver` derives the real query text from the pipeline's own real, required `requirement` input — see this document's Implementation Status above for the full design (why `requirements-analyst`, not `documentation`; why a pipeline-local derivation, not a new Step Contract field). Not yet wired into `se.product_creation` or any other workflow.

**The "Memory Resolver" bullet above is now real too (2026-08-04, `P02-S03-M08-T06`).** `ai_os_kernel.context_manager.resolvers.MemoryResolver` calls the real Memory Store (§7's own related component, `memory_manager.md`) — its first genuine consumer. Genuinely cross-run: proven against real Postgres that a memory item written under one workflow instance surfaces in a context assembly for a *different* one, never filtered by `source_workflow_id`. Wired into a real production composition later, as of `P02-S03-M08-T13` (see below).

**`MemoryResolver` reaches a real production composition too (2026-08-05, `P02-S03-M08-T13`) — the third of the four resolvers to close the "not yet wired" gap, and the simplest.** No per-step query derivation needed (unlike `KnowledgeResolver`'s `T12`) and no environment-validation failure mode to degrade from (unlike `RuntimeConfigResolver`'s `T11`) — a plain, unconditional `SqlMemoryStore(engine)` read. Added to the identical two real compositions `T11` already extends (`bootstrap._lifespan`, `bootstrap.build_workflow_worker_loop`), alongside `WorkflowStateResolver`. `memory_type="engineering"` — memory_manager.md's own documented, cross-run, long-lived category, matching this resolver's own "not scoped to one workflow" design. Proven end to end against real Postgres (`tests/integration/test_bootstrap_workflow_trigger.py`): the real `_build_workflow_trigger` path, given the real composition `_lifespan` now builds, genuinely renders a real, cross-run memory item — written under one workflow instance — into a *different*, later-triggered instance's own real agent output, alongside that run's own real `inputs`.

**`AIContextPackResolver` — the fourth and last resolver — is genuinely, correctly blocked from production wiring (2026-08-05, `P02-S03-M08-T14`), not merely undone.** Unlike the other three, no real content exists to wire against: `ai_context/` has zero tracked packs anywhere in this repository (`../../ai_context/ai_context_strategy.md`'s own Implementation Status: "Built: nothing... zero Context Packs of any type have been written"). A `pack_references=()` wiring would contribute nothing while being presented as real production value — exactly the forced, empty wiring this step's own scope explicitly forbade. Authoring the first real pack is a curatorial content decision (`ai_context_strategy.md` §5: "curated over complete... obsolete or misleading context is worse than missing context"), not an engineering one a resolver-wiring ticket should invent — the identical class of "real, disclosed prerequisite outside this ticket's own authority" `P02-S04-M10-T03` (Promotion Logic) already established. Blocked on: at least one real, authored, curated AI Context Pack, and a decision on which real composition(s) should reference which packs (no `pack_references`-equivalent configuration surface exists yet, unlike `capability_pack_dirs` for Capability Packs).
...
**`KnowledgeResolver` closes the same gap too (2026-08-05, `P02-S03-M08-T12`), into `se.delivery_pipeline`'s `requirements-analyst` step — not `documentation`, the originally-suggested candidate.** Investigation found `documentation`'s own contract (`DocumentationAgentEntrypoint._extract_payload`) requires the *entire* assembled context to be one parseable JSON object — adding a second, free-text `KnowledgeResolver` item there would break `json.loads()` on every real invocation, not merely contribute nothing. `requirements-analyst` is the real, safe target: its agent already free-text-flattens however many context items it receives into one `{{context}}` prompt variable, the identical safe shape `PromptedAgent` uses. A second, real gap: nothing in production ever set `ContextRequest.knowledge_query` (the documented Step Contract has no query-text field), so wiring `KnowledgeResolver` in anywhere would have resolved to `[]`. Resolved with a pipeline-local `ai_os_kernel.workflow_engine.delivery_pipeline._QueryFromRequirementInputResolver`, deriving the query from the pipeline's own real, required `requirement` input (`delivery_pipeline.yaml`'s own `inputsSchema`) — zero change to `WorkflowStep`/`AgentStepExecutor`/the documented Step Contract, a deliberate, narrower alternative to a general `knowledgeQuery` field, chosen over it. `bootstrap._build_knowledge_resolver` builds the real `Embedder`/`QueryEngine` pair, degrading to `None` (matching `RuntimeConfigResolver`'s own precedent) when no local embeddings server is configured — true for a fresh checkout. Proven against real Postgres and a real local embeddings server (`tests/integration/workflow_engine/test_delivery_pipeline_knowledge.py`): the real `build_pipeline_trigger` production path, given the real composition `_lifespan` now builds, genuinely renders a real indexed document's content into `requirements-analyst`'s own real, persisted output, alongside the pipeline's own real `requirement` input; a `knowledge_resolver=None` run (every caller before this step, and any real deployment without a configured embeddings server) is unchanged.
