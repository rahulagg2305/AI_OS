...
## Implementation Status (2026-07-28; Prompt Resolver correction + Composition/inheritance added 2026-08-05; Cache boundary added 2026-08-05)
...
**As of `P02-S03-M07-T06`**, §12's own `cache_boundary_index` is real too — `cache_boundary.py`'s `split_at_cache_boundary()`/`render_with_cache_boundary()`. No heuristic detects the split from arbitrary text (ADR-0025 §2 describes what stable/volatile means, not how to detect it automatically, and none of this codebase's own precedent invents unfounded heuristics — see `P02-S04-M10-T03`'s own finding); instead, a real, disclosed `{{cache_boundary}}` marker the template's own author places explicitly, consistent with `render_template()`'s existing `{{variable}}` syntax. The reported index is computed on the real *rendered* text (each side rendered separately through the unchanged `render_template()`), not the raw template — a `{{variable}}` before the boundary substituting to a different length would otherwise silently report the wrong index. A standalone utility, like `composition.py` — no change to `PromptRenderResponse` or either real `PromptEngine` implementation.
