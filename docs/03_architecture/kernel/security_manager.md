...
**Updated (2026-08-07, `P07-S02-M14-T01`): real OIDC verification now exists, closing this section's own previously-named gap.** `security_manager.token_verifier.OidcBearerTokenVerifier` verifies a JWT against a real, JWKS-fetched RS256 public key (`jwt.PyJWKClient` — real HTTP fetch, cached, run off the event loop thread) and validates issuer and audience, not merely signature and expiry — the exact three gaps this section used to name ("no identity provider, no JWKS, and no issuer/audience validation") are each genuinely closed. `bootstrap._build_token_verifier` selects it only when all three real fields (`PlatformConfig.oidc_issuer`/`oidc_audience`/`oidc_jwks_uri`) are configured together; every current environment, which configures none of them, keeps the pre-shared-secret `JWTBearerTokenVerifier` unchanged — a real, deliberate, zero-regression choice, not the literal "replace" the ticket's own title suggests, since removing the pre-shared-secret path would break every environment before any real OIDC provider is actually configured anywhere. Proven against a real, self-hosted JWKS HTTP server and real RS256-signed tokens (no live third-party identity provider is configured in any environment yet — the identical "real mechanism, self-hosted rather than managed" shape `postgres_container()` already establishes for Postgres).

**Updated (2026-08-07, `P07-S02-M14-T02`): "Full five-role model" — a real, persisted role grant now takes effect for every permission-checked route, not only `ApprovalService.decide()`.** All 5 documented roles (`viewer`/`operator`/`approver`/`maintainer`/`admin`) were already correctly modeled/enforced via bearer-token-claimed roles, and role administration's persistence/HTTP-route layer (`P03-S05-M14-T07`/`T08`, described above) was already fully generic (any `role: str`, not restricted to the `approver:<class>` family) — the real, remaining gap was that a persisted grant only ever took effect inside `decide()`'s own narrow check. The union (`principal.roles | active persisted grants`) is now a shared `resolve_effective_roles()` (`security_manager.role_administration`), reused unchanged by `decide()` and newly by the general `authenticate()` FastAPI dependency, reading an optional `request.app.state.role_grant_repository` (wired in `bootstrap._lifespan`; `None` for any caller with no real database, identical behaviour to before this ticket). Proven end to end, real Postgres, real HTTP: a principal whose bearer token carries no roles at all is refused `GET /api/v1/packs` (`pack:read`, granted only to `maintainer`/`admin`), a real, persisted `maintainer` grant for the identical, unchanged token then genuinely enables the identical request, and a real revoke disables it again.
