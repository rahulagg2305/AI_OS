...
## Implementation Status (2026-08-01, updated 2026-08-12)

**Built:** `kernel/src/ai_os_kernel/manifest_loader/` (4 modules). `discovery.py` performs **both ADR-0009 discovery mechanisms**: `discover_manifests()`, the filesystem scan over configured pack directories, and, as of `P01-S03-M02-T03`, `discover_entry_point_manifests()`, the `ai_os.capability_packs` entry-point mechanism — installed distributions are enumerated via `importlib.metadata.entry_points()`, and each one's `manifest.yaml` is located through its own recorded distribution files (`Distribution.files`/`locate_file()`), never by importing the entry point's own target (§6: "must not execute domain logic from the pack"). `ManifestLoader.scan()` in `loader.py` combines both mechanisms and de-duplicates identical resolved paths, so §3's "both validated identically" is no longer vacuous. `ManifestLoader` does YAML parsing, **JSON Schema validation against the real `platform_sdk/schemas/manifest.schema.json`** (Draft 2020-12, with the schema itself checked at construction time), and `metadata` extraction. Two entry points with deliberately different failure semantics, both real: `load_one()` **fails closed** — an invalid manifest raises `ManifestError` and is never returned — while `scan()` never raises, capturing each failure into a `ManifestLoadFailure` so one broken pack cannot hide every other pack from status reporting. `models.py` holds `PackMetadata`, `DiscoveredManifest`, `ManifestLoadFailure`, `ManifestScanReport`. Built in step 3 of `kernel/src/ai_os_kernel/bootstrap.py`. **Added 2026-08-12 (`P01-S03-M28-T02`, FR-117): `signature.py`** — detached Ed25519 verification of `<manifest>.sig` against a PEM trust store resolved from `PlatformConfig.manifest_trust_store_dir`, over the SHA-256 of the manifest's canonical JSON. Verification runs on every load and its outcome is recorded on `DiscoveredManifest.signature` even when enforcement is off, so the signing state of the estate is observable before anyone enables it. Four honest outcomes — `signed_and_valid`, `unsigned`, `invalid`, `unverifiable` — and with `require_signed_manifests` on, anything but the first is refused by `load_one()`'s existing fail-closed path, before installation or activation. Default off, so all three committed packs (all unsigned) are unaffected; see `../../09_security/security_architecture.md` §8 for the three decisions and why each alternative was rejected. The JSON Schema itself is the most complete artifact in this area — real, versioned, and actively enforced.

**Not built** (**two claims in this paragraph were stale and are corrected inline, 2026-08-12**). It opened "No semantic validation at all... none of the 10 semantic rules (12–21) is enforced" — **false**: `semantic.py` exists and `load_one()` calls both `validate_sdk_version_range` and `validate_workflow_definitions` on every manifest, so the `sdkVersion` range check (once unenforceable, since the SDK package now exists) and workflow step-reference resolution are both real. What remains genuinely unenforced of those rules: no kernel-version compatibility check, no cross-pack global ID uniqueness, no reference resolution, no dependency resolution, no circular-dependency detection, and **no permission-subset checks** — so §3's "agent permissions are a subset of every workflow using them" and the platform's monotonic-narrowing rule are not verified at install time or at runtime. **Forbidden-import verification is real** (the second corrected claim — this read "does not run... gated on `scripts/check_import_boundaries.py`, which does not exist"). That script exists and CI runs it (`ci.yml`'s "check import boundaries" step); §3's "no provider SDK, no `ai_os_kernel`, no other pack" check genuinely executes against every pack. It is enforced in CI rather than by this loader at install time, which remains a real difference from §5's flow. **No Component Registrar** — nothing writes `catalog.agents` / `catalog.tools` / `catalog.workflow_definitions` rows from a manifest, and nothing calls a pack's `entryPoint`; those rows are currently written directly by other code paths. **No Audit Logger** (`governance.audit_log` has no writer anywhere for this component), so §8's per-attempt record does not exist. Of §5's ten loading-flow steps, only 1–4 are real; steps 5–10 are unimplemented. Of §4's nine components, only Pack Discovery, Manifest Parser, Schema Validator and Error Reporter exist.
