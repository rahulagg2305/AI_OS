...
- Discover packs by **entry point** (`ai_os.capability_packs`, the production mechanism) and by **filesystem scan** of configured directories (the development mechanism) — both validated identically ([ADR-0009](../../18_decision_log/adr/ADR-0009-packaging-and-dependency-management.md)). *(Only the filesystem scan exists today — see Implementation Status.)*
...
| Item | Status |
|---|---|
| **Discovery mechanisms** | **Decided, half built.** [ADR-0009](../../18_decision_log/adr/ADR-0009-packaging-and-dependency-management.md) fixes both mechanisms and which is which: the `ai_os.capability_packs` Python entry point for production, a filesystem scan of configured directories for development, validated identically. There is nothing left to choose. **Named remaining gap:** entry-point discovery is unimplemented — `kernel/src/ai_os_kernel/manifest_loader/discovery.py` only walks directories. This is a small, self-contained increment (`importlib.metadata.entry_points`) blocked by nothing. |
