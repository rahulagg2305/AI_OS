...
**`POLL_INTERVAL_SECONDS` is now genuinely enforced (2026-07-30) — the Pack Health Collector re-polls continuously, not just once at startup.** `ai_os_kernel.capability_manager.health_poller.run_health_polling_loop()` — the first real, continuously-running background task this Kernel has ever started — sleeps for the real, decided interval, then re-polls every discovered pack, forever, until cancelled; started in `_lifespan` immediately after the existing one-shot startup poll (unchanged), stored at `app.state.pack_health_polling_task`, and genuinely cancelled and awaited in `_lifespan`'s own `finally` block *before* the database engine it queries is disposed — no orphaned task, no query racing a closed connection pool. A new, test-only `PlatformConfig.pack_health_poll_interval_seconds` field (`None` by default — every real deployment uses the real constant unchanged) lets a test run the loop against a short interval instead of waiting out the real 30-second cadence. **Proven end to end against real Postgres containers** in `tests/integration/test_health_polling_loop.py`: `catalog.packs.health.checked_at` genuinely, strictly increases across three independent real waits, well after the one-shot startup poll; the background task is genuinely `done()`/`cancelled()` immediately after a real `TestClient` shutdown, with no additional wait needed to observe it. This closes the very last piece of capability_manager.md §9's own three-value policy — poll interval is now decided *and* enforced, not merely decided.

**§6's Shutdown Coordinator is now real (2026-08-01, `P01-S04-M03-T06`) — no longer "one narrow instance," but one general mechanism covering every Kernel background loop.** `ai_os_kernel.health.shutdown.GracefulShutdownCoordinator` replaces the two duplicated cancel-and-await blocks `_lifespan` previously carried inline (one per loop) with a single, independently unit-tested class supporting two real stop shapes: hard cancellation (the Pack Health Collector and Lease Reaper loops, whose own iterations have no cooperative stop signal) and stop-event drain (a loop that finishes its current pass before exiting on its own — gentler, and now what the scheduled audit-chain verification job uses). **This is also the first time that audit-chain verification job (`ai_os_kernel.observability.audit_verification_job`, built and unit-tested in `P01-S05-M04-T06`) has run in a real Kernel process at all** — it existed but nothing ever started it until this step wired it into `_lifespan` alongside the other two loops, all three now drained through the one coordinator. A real, discovered gap fixed along the way: this job's own per-pass logic had no resilience against a genuine database failure (unlike the other two loops, which already logged and continued past one) — a single transient error would have silently killed the whole job for the rest of the process's life, discovered only once the job was wired into a live process for the first time; fixed to match the established per-pass try/except pattern. **Proven** in `tests/unit/kernel/health/test_shutdown.py`: a stop-event job's in-flight iteration genuinely completes before the loop exits (not interrupted mid-step), a cancelled job is genuinely stopped and awaited, and multiple jobs drain concurrently, not sequentially. **Still not** the full breadth of §3's/§9's "safe startup and graceful shutdown coordination across every Kernel component": in-flight HTTP request draining is delegated to the ASGI server (Uvicorn's own graceful-shutdown handling, which completes before `_lifespan`'s shutdown phase ever runs) and is not this module's own concern.
