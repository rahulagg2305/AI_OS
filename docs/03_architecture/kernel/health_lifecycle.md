...
**Not built:** of §6's six elements only the Health Aggregator and the Readiness/Liveness Interface exist. There is **no Kernel Lifecycle Controller**: §4's six states (`Starting`, `Ready`, `Degraded`, `Stopping`, `Stopped`, `Failed`) are documented vocabulary with no state machine and no persistence — `ready`, `degraded`, and (since this step) `not_ready` are the only three `ReadinessReport.status` values ever produced, and `/health/ready` now genuinely returns HTTP 503 for `not_ready` (a real hard dependency, the database, now justifies refusing traffic) and HTTP 200 for the other two. There is **no Shutdown Coordinator** and no graceful-shutdown path, so §3's "safe startup and graceful shutdown" and §9's shutdown events do not happen. There is **no Pack Health Collector**: `HealthReport` exists as a model in `kernel/src/ai_os_kernel/capability_manager/pack_contract.py` but nothing produces or collects it, so §5's "activated Capability Packs status" (now partially covered by the real activation-status check above, though not through a `HealthReport`) and §7's "Capability Pack health is reported through the Capability Manager and aggregated here" remain otherwise unimplemented. There is **no Observability Hook**: no health metric, no event, and no log line on a state transition — the Event Bus this would publish to is itself an empty stub. §5's "ability to accept new workflows" is not reported.
...
| **Health check protocols** | **Kernel-internal protocol decided (revised 2026-07-30); the pack protocol is open.** Internally, a check is a callable returning `ComponentStatus` either directly (synchronous) or as an `Awaitable` (asynchronous, added 2026-07-30 so a check can genuinely read real database state), registered in `bootstrap.py` ([ADR-0010](../../18_decision_log/adr/ADR-0010-composition-and-dependency-injection.md): explicit composition, no container) — that is settled and built. **(a) Hard vs. soft dependency — DECIDED and BUILT (2026-07-30): the database is hard.** Evidence: every functional route in this codebase (`POST /api/v1/workflows`, `.../se.delivery_pipeline`, `POST`/`GET /api/v1/packs*`) already independently returns 503 the moment its own database-backed `app.state` object is absent — a Kernel reporting `200 degraded` while every functional route 503s anyway is the exact "worse than refusing traffic" case this row used to warn about. `ComponentStatus.critical: bool` + `ReadinessReport.status`'s new third value, `"not_ready"`, implement it generically (any future hard dependency reuses the same mechanism, not a one-off if-statement); `database_check` (`ai_os_kernel.bootstrap._build_health_service`) is the one check marked `critical=True` today, backed by a real, timeout-bounded `SELECT 1` — see the Implementation Status section above for the full record and `tests/integration/test_health_database_check.py` for the real, both-directions proof. **Remaining gap, now down to one part.** (b) *Pack health polling:* the same undecided three-value policy named in `capability_manager.md` §9 — poll interval, timeout, and the consecutive-failure count that moves a pack to `failed`. A self-contained decision that blocks no other work and is blocked by no other component. |
