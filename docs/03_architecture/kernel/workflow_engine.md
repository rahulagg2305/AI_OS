...
- **A third real instance of that "future worker process framework" now exists (2026-08-02, `P02-S01-M05-T12`): a multi-instance worker loop.** Every real advance path before this step (`WorkflowInstanceService.advance`, `WorkflowAdvanceRunner.run_once`/`run_to_completion`) required a caller to already name one specific `workflow_id`. `WorkflowWorkerLoop`/`run_worker_loop` (`worker_loop.py`) is the first real mechanism that *discovers* which instances are runnable and drives many of them concurrently, via a new, purpose-built `WorkflowInstanceRepository.list_runnable_instances` query (`status = 'running'`, no active lease — genuinely different from `list_instances`'s own paginated, newest-first listing for api_architecture.md §9). Each tick calls `WorkflowAdvanceRunner.run_once` for every discovered instance via one real `asyncio.gather` — one step per instance per tick, not `run_to_completion` per instance, so no single long-running instance can starve the rest of a batch. Reuses the identical `P02-S01-M05-T11` composition-level-injection precedent for definition resolution (`WorkflowDefinitionCatalog` is still write-only), keyed by the full `(definition_id, definition_version)` pair rather than id alone — a worker loop can discover instances spanning more than one still-running version, unlike `SubWorkflowStepExecutor`'s narrower, single-version-in-scope case. **Real and proven — not wired into `_lifespan`.** Unlike the reap loop, this is deliberately *not* started in the running Kernel process yet: doing so would need to answer "every currently-registered `WorkflowDefinition`, system-wide," the same write-only-catalog gap this step's own definitions mapping sidesteps at the composition level for a caller who already knows its own definitions — a real, undecided question this ticket's own Input/Output ("Runnable instances" → "A scheduling loop across instances") does not ask it to resolve, so it is left real, tested, and available to a composition that does, the identical "proven, unused" precedent `decision`/`parallel`/`sub_workflow`/`quality_gate_executor` already established above.
