...
- **Not built at all** (no matching class anywhere in the Kernel): Gate Coordinator (§5.8), Human Approval Manager (§5.9), Failure & Retry Manager (§5.10), Event Publisher (§5.11), Scheduler (§5.13, delayed/scheduled workflow starts — distinct from the Lease Manager's own now-real reap scheduling above). A failing step today raises an exception; nothing decides retry/compensate/escalate, no gate is evaluated, no human pause happens, no event is published, and no delayed/scheduled start exists.
