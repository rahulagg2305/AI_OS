...
**What §4.1's "current status" concretely is, verified against `kernel/src/ai_os_kernel/workflow_engine/instance.py`:** the canonical list is 9 states. **Correction (2026-08-08, found during a full pre-completion health audit): this line was stale — `waiting_for_human` has genuinely been reached since 2026-08-02 (`P03-S05-M14-T04`/`T05`), five days before this line was last verified.** **5 states are ever written** — `created`, `running`, `waiting_for_human` (via `WorkflowInstanceRepository.mark_waiting_for_human`, a real CAS-guarded transition off `running`), `completed`, and, as of 2026-08-10 (`P06-S01-M36-T04`), `cancelled` (via the new `WorkflowInstanceRepository.cancel`/`POST /api/v1/workflows/{id}/cancel`, the identical guarded-CAS shape, real only from `created`/`running`/`waiting_for_human` — the three real states any writer today ever produces). **Updated 2026-08-13 (`P02-S01-M05-T17`): `failed` is now genuinely written too, making it 6 of 9.** The multi-instance worker loop transitions an instance to `failed` once its step exhausts the retry budget (`WorkflowInstanceRepository.mark_failed`, the same guarded-CAS shape as `cancel`) — closing R-016, where a permanently-failing step retried forever because the lease was released, the status stayed `running`, and the next poll rediscovered it. `waiting_for_retry`, `quality_gate_failed` and `compensating` remain declared in the enum with no code path transitioning an instance into any of them. **`waiting_for_retry` is now unreached by an explicit decision rather than by omission (2026-08-13, `P06-S01-M36-T05`):** `POST /api/v1/workflows/{id}/retry` is the one natural writer it will ever have, and the product owner chose to send a retried instance **straight to `running`** instead. The reason is concrete — `running` is non-terminal, so the existing `list_runnable_instances` filter rediscovers the instance with no change to the worker loop, whereas routing through `waiting_for_retry` would have required teaching that loop's discovery queries and its startable/runnable split a new state, widening the blast radius on the very loop `P02-S01-M05-T17` had just stabilised. The retry is still fully observable: it writes a real `state.transitioned` event and stamps `workflow_instances.retried_at`. §5's "Idempotency of steps is strongly preferred" is real and enforced — `(workflow_id, step_name, attempt)` is a unique constraint on `workflow_steps`.
