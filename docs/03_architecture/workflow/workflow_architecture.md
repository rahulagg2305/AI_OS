...
**Each item becomes the child instance's own real `inputs` — no field mapping or transform.** `ForeachStepExecutor` passes each real list element straight through as `WorkflowInstanceService.create_instance(inputs=item, ...)`; whether that item's own shape satisfies the target sub-workflow's own declared `inputs` schema is a contract between whichever step produces the plan artifact and whichever sub-workflow `subWorkflowId` names — not something this step type mediates. This is a real, discovered constraint, not a hypothetical one: chaining `technical-planner`'s own real plan-item shape (`taskId`/`title`/`description`) into `se.implement_task`'s own real `inputs` schema (`{task: string}`, `additionalProperties: false`) fails validation as-is — see `P08-S02-M30-T01`'s own ticket for the two real options this leaves open (widen `se.implement_task`'s schema, or add an explicit per-item field-selection to this contract).
