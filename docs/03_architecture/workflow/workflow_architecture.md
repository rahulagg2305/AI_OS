...
- **2.5 of the 7 Supported Step Types genuinely execute (updated 2026-07-30).** `agent` and `tool` steps dispatch to real executors; `decision`, `parallel`, `sub_workflow`, and `human_approval` still route to `NoOpStepExecutor` and complete having done nothing. `quality_gate` is now real for exactly one, narrow, composition-configured case — see `../kernel/quality_gate_engine.md`'s own Implementation Status — not a general mechanism; a caller that doesn't configure one still routes to `NoOpStepExecutor` unchanged.
