...
- **All 7 Supported Step Types now genuinely execute when configured (updated 2026-08-02, `P03-S05-M14-T04`/`T05`) — the last, `human_approval`, closing R-001's own permanent hard rule (`risk_register.md`).** `agent` and `tool` steps dispatch to real executors; `decision`, `parallel`, `sub_workflow`, and now `human_approval` do too. `DecisionStepExecutor` genuinely evaluates a real, closed-vocabulary condition (see the "Decision Step Contract" below) against a named prior step's own persisted output and branches execution to one of two declared targets, not a positional walk. `ParallelStepExecutor` genuinely runs its declared branches concurrently (real `asyncio` tasks, proven by real wall-clock timing, not a sequential loop) and joins per the real, closed-vocabulary "Parallel Step Contract" below — `all`/`any`/`collect`, exactly as `workflow_engine.md` §7.1 already documented the *policy* semantics, though the *membership* contract (which steps are a parallel step's branches) was, like decision-step branching, undocumented anywhere until this step. `SubWorkflowStepExecutor` genuinely creates a real, separate child `WorkflowInstance` via its own, independent `WorkflowInstanceService`/`WorkflowAdvanceRunner`, runs it to completion, and joins on that child's own real, persisted last-step output — see the "Sub-workflow Step Contract" below for the `subWorkflowId` field and why definition resolution is composition-level, not a catalog read. `HumanApprovalStepExecutor` genuinely, durably pauses the real instance (`waiting_for_human`) at a declared approval point and resumes only on a real, attributable decision — never a timeout — see `../governance/human_approval_points.md`'s own Implementation Status for the full design and its real, disclosed scope limits (no HTTP route, no RBAC yet). `quality_gate` is real for exactly one, narrow, composition-configured case — see `../kernel/quality_gate_engine.md`'s own Implementation Status — not a general mechanism; a caller that doesn't configure one still routes to `NoOpStepExecutor` unchanged. Like `quality_gate`/`decision`/`parallel`/`sub_workflow`, no real composition wires `human_approval_executor` in yet either — the mechanism is real and proven, unused by any running pipeline today. **An 8th Supported Step Type, `foreach`, was added and made real 2026-08-09 (`P08-S02-M30-T01`)** — see the "Foreach Step Contract" below; `ForeachStepExecutor` reuses `SubWorkflowStepExecutor`'s own per-item mechanism sequentially, bounded by a required `maxFanOut`. **Wired into a real, running composition the same day, later step:** `se.product_creation`'s own step 8 (`implement-tasks`) genuinely fans `technical-planner`'s own real plan out into real `se.implement_task` children — see the Foreach Step Contract's own closing note for the real schema-mismatch fork this took to resolve.
...
**Resolved via composition-level injection, not a catalog read.** `SubWorkflowStepExecutor` is constructed with a plain `{workflow_definition_id: WorkflowDefinition}` mapping — the identical shape `QualityGateStepExecutor`'s own `gate_sources` and `WorkflowAdvanceRunner.run_to_completion`'s own `step_retry_targets` already establish for "a cross-step/cross-workflow reference belongs in the composition layer, not as new, workflow-file-facing architecture." A step whose declared `subWorkflowId` is absent from this mapping fails clearly (`SubWorkflowFailedError`), never silently skipped. **`WorkflowDefinitionCatalog` is no longer write-only** (`P02-S01-M05-T14` added a real `get(definition_id, version)` reader) — `SubWorkflowStepExecutor` itself was deliberately left unchanged, still resolving via this same composition-injected mapping; switching it to the new reader is real, disclosed follow-up work, not required by anything documented here.
...
## Foreach Step Contract

**Added 2026-08-09 (`P08-S02-M30-T01`), closing ADR-0021's own "dynamic decomposition without dynamic control flow" pattern (§1 above: "a declared `foreach` step executes a declared sub-workflow per item").** Reuses two contracts this document already establishes rather than inventing new ones: `DecisionCondition`'s own "one named source step, one field of its output" shape (for finding the plan artifact), and `sub_workflow`'s own `subWorkflowId` reference (for which definition to run per item).

A `foreach` step declares two fields, both required together:

| Field           | Description                                                                                     |
|-----------------|---------------------------------------------------------------------------------------------------|
| `subWorkflowId` | The id of the child `WorkflowDefinition` to invoke once per item — the identical field and resolution mechanism (composition-level `{id: WorkflowDefinition}` mapping) the Sub-workflow Step Contract above already establishes |
| `foreach`       | `sourceStepId` (a step declared earlier in the sequence), `itemsField` (a key in that step's own recorded output naming the real list), `maxFanOut` (a required, positive integer bound — ADR-0021 point 3: "`foreach` declares a maximum fan-out") |

**Bounded, per ADR-0021's own "no unbounded agent-driven loop" rule.** The real item count at `sourceStepId`'s own `itemsField` is checked against `maxFanOut` *before* any child instance is created — exceeding it refuses the whole step (`ForeachFailedError`), never silently truncates the list. There is no default: every `foreach` step must state its own bound explicitly.

**Sequential per-item execution, not concurrent — a real, disclosed, narrower scope than the Parallel Step Contract above.** ADR-0021 names `foreach` as a bounded fan-out over a plan artifact, not a concurrency primitive; `ForeachStepExecutor` runs `SubWorkflowStepExecutor`'s own real per-item mechanism (create, start, run to completion, join on the child's own last real persisted output) once per item, in order. Concurrent fan-out remains real, unclaimed follow-up work if ever needed — `ParallelStepExecutor` already proves the concurrency machinery exists in this codebase, just not wired to `foreach`.

**Each item becomes the child instance's own real `inputs` — no field mapping or transform.** `ForeachStepExecutor` passes each real list element straight through as `WorkflowInstanceService.create_instance(inputs=item, ...)`; whether that item's own shape satisfies the target sub-workflow's own declared `inputs` schema is a contract between whichever step produces the plan artifact and whichever sub-workflow `subWorkflowId` names — not something this step type mediates. This is a real, discovered constraint, not a hypothetical one: chaining `technical-planner`'s own real plan-item shape (`taskId`/`title`/`description`) into `se.implement_task`'s own then-real `inputs` schema (`{task: string}`, `additionalProperties: false`) failed validation as-is. **Resolved the same day, later step (`P08-S02-M30-T01`, `AskUserQuestion`): the consumer's schema was widened to match the real producer** — `se.implement_task`'s own `inputs` now declare `title`/`description` directly (matching `technical-planner`'s own `PlanTask` verbatim), rather than adding a second, per-item field-mapping mechanism to this contract for a need seen exactly once so far. `se.product_creation`'s own step 8 (`implement-tasks`) is the real, proven, first user of this exact "pass the item straight through" contract.
...
- [`../governance/human_approval_points.md`](../governance/human_approval_points.md) — the Human-in-the-Loop contract (real, minimal pause/resume built — see its own Implementation Status for what remains)
