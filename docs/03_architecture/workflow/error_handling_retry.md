...
- **A per-exception-type retriable/non-retriable classification exists only where each exception's own author has added it** (`QualityGateFailedError`, `LLMProviderError`/`LLMRefusalError`) — there is no central, enforced taxonomy requiring every new Workflow Engine exception to declare one; `AgentRegistryError`/`ToolRegistryError` are named, deliberate exceptions (each conflates a transient and a permanent nature in one type with no structural way to tell them apart — defaulting to non-retriable, `advance_runner.py`'s own module docstring has the full reasoning). Compensation/rollback (§6) and human escalation (§7) do not exist; `security` errors have no producer, since the Security Manager's own audit path is largely unbuilt.
