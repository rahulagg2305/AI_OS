...
**Version:** 2.4
**Status:** Approved — 18 steps; **Steps 1, 2, 2a, 3, 4, and 5 complete**, Step 6 next
**Last Updated:** 2026-07-29 (v2.4: **Step 5 complete** — `PromptRegistry` Protocol built at the documented keyword call style (the one reversal decision from 2a), `RenderedPrompt` narrowed to 3 fields, and the conversion to/from the real Kernel's `PromptRenderRequest`/`PromptRenderResponse` proven lossless in both directions since no real Kernel class satisfies this from-scratch shape directly. Record in §6e. Prior, v2.3: **Step 4 complete** — `LLMGateway` Protocol built narrowed to `complete()`/`capabilities()`, `ProviderCapabilities` extended to the real 13 fields, structural compatibility proven against the real `DispatchingLLMGateway`. Record in §6d. Prior, v2.2: **Step 3 complete** — `Agent` and `Tool` Protocols built at the narrowed shapes, proven against all five real pack agents and both real tools. Record in §6c. Prior, v2.1: **Step 2a complete** — all five Protocol/reality reconciliation decisions recorded as dated blocks in `platform_sdk.md`; summary in §6b. Prior, v2.0: an independent architecture review found 7 real problems — three blocking — in v1.0's 15-step sequence; sequence revised to 18 steps. Findings: §6a.)
...
| 6 | `ToolInvoker` Protocol + `ToolDescriptor` + **`ToolResult`** (moved here from step 3, since this is where its consumer lives), and the `platform.sandbox.run_command` tool contract — all as decided in 2a. **`SecretResolver` removed from this step** — see §2.3. **Next.** | |
