...
| 5.6 | `ToolInvoker` | **Stale row — real since step 6, extended `P02-S05-M18-T03`.** `ai_os_sdk.contracts.tool_invoker.ToolInvoker` is a real, implemented Protocol; `ToolInvokerAdapter` now resolves and invokes both the platform sandbox shim and any manifest-declared `catalog.tools` entry through the real `ToolRegistry` — see §5.6's own decision block below for the full, current picture | `available_tools()` still cannot enumerate pack-declared tools (no such lookup exists on `ToolRegistry`); permission enforcement is real for the pack-grant term only, not the full principal chain |
