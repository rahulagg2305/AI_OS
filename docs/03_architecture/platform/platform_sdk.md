...
| 5.5 | `MemoryService` | **Stale row — real Kernel-side since `P02-S04-M10-T04` (2026-08-14).** `ai_os_kernel.memory_manager.service.MemoryService` implements §6's mediated `write()` over the real `SqlMemoryStore`, with §8's observability requirements given their first producer | The SDK Protocol itself is still deliberately deferred (`platform_sdk_v1_scope.md` §7), so this is Kernel-local and no pack can reach it. `recall()` is deliberately absent — memory is read through the Context Manager's `MemoryResolver` (§6/§7), not a second read path. **No production caller writes memory yet**: no document decides the trigger, so the write path is real but unwired (product-owner decision, 2026-08-14) |
...
| 5.7 | `EventBus` | **Stale row — real Kernel-side since `P02-S07-M17-T02`/`T03`/`T04`.** `ai_os_kernel.event_bus` holds a real `InProcessEventBus`, a real `OutboxRelay` running in `bootstrap.py`, and `write_outbox_event`; `platform.event_outbox` is no longer unused — the Workflow Engine's terminal `workflow.completed` transition writes to it in the same transaction | The SDK Protocol itself remains deferred (`platform_sdk_v1_scope.md` §7), so no pack can reach it. Topic/Channel Manager and Schema Registry are still unimplemented |
