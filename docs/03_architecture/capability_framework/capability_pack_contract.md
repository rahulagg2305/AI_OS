...
- **The SDK now exists, but no pack has migrated onto it — updated 2026-07-29.** `ai-os-sdk` is real as of `platform_sdk_v1_scope.md` steps 1–8 (`../platform/platform_sdk.md` §11 has the current, accurate status), so "interact only through the Platform SDK" is now genuinely enforceable — but the one real pack still knowingly violates it, under a real, mechanical, expiring waiver (see the dated exception under Platform Interaction Rules below).
- **The SDK pack contract suite is partially real, as of step 8: check 7 (forbidden imports) only.** `ai_os_sdk.testing.forbidden_imports`/`waiver` genuinely enforce "no forbidden dependencies" in CI today — the other 8 checks in the Testing Requirements and Validation Checklist below remain review-enforced, not tool-enforced, until step 15 builds them against the by-then-migrated pack.
