...
- **Updated (2026-08-02, `P03-S01-M24-T03`): a second, distinct declared-permission check now enforces too — capability *grant*, not just the pack-ceiling check above.** `ai_os_kernel.sdk_adapters.pack_context.build_pack_context` now forwards a real `GitIntegrationService` into an entrypoint's `ToolInvokerAdapter` only when that entrypoint's own declared `permissions` include `git:write` (**not** merely `sandbox:execute`, the real, confirmed gap this step closes — every `sandbox:execute` agent in the Software Engineering pack, not only `git-push`, previously received a working `git_service` whenever the registry had one configured at all). Proven both directions with the identical real `GitIntegrationService`: `git-push`'s own declared `[sandbox:execute, git:write]` genuinely commits and pushes; `lint`'s/`qa-test`'s own declared `[sandbox:execute]` alone gets `UnknownToolError` on every Git tool id, with zero audit rows for the attempt (refused before the service is ever reached). Still the declared-permission term only, not the full principal-inclusive chain — identical scope to T5's own pack-ceiling check above, for the same reason (no `SecurityContext` reaches this call site).
- **Resolved (2026-08-01, `P01-S06-M42-T04`): `tests/security/` now holds one test file per threat (T1–T12), 30 tests total, each exercising that threat's real, current defense against a genuine attempt.** For T11 (no Git Integration Service exists), the tests remain a disclosed absence-of-capability tripwire, not a positive-control proof. **T10 is different as of 2026-08-02 (`P03-S05-M14-T04` through `T06`): `test_t10_unauthorized_approval.py` is now a real, positive-control proof for all three of §9's own named defenses** ("approval classes, attributable decisions, timeout never approves") — a real `Approval` row is attributable (`decided_by`/`decided_at`), a real declared `timeout` genuinely elapsing never implies approval, and `ApprovalService` genuinely refuses a decision from a principal lacking the ADR-0023-documented class-scoped `approver:<approval_class>` role (or `admin`), proven against both fakes and a real Postgres row. Still disclosed, deferred scope: no HTTP route/Bearer-token wiring for this call site, and no role *administration* (who may grant `approver:<class>` itself is unbuilt).
...
- **Updated (2026-08-02, `P03-S01-M24-T01`): §14 Git Safety has a real enforcing component now — the Git Integration Service (`kernel/src/ai_os_kernel/git_integration/`).** The protected-branch push prohibition is real (structurally absent from the tool surface, not gated, per `git_integration.md` §5.1); the mutating-operation audit trail is real (every commit/branch/push records a real, hash-chained `governance.audit_log` row); credentials never enter a sandbox (a resolved secret travels only via a per-call `GIT_ASKPASS` script, never a URL/config/log/audit payload). No clone/fetch (this service's own disclosed scope) and no history-rewriting prohibition beyond "not built" (this service exposes no such operation at all). Full detail: `git_integration.md`'s own Implementation Status.
- **Stale by the same day (`P03-S04-M31-T04`/`P03-S03-M30-T04`/`P03-S01-M24-T02`): the "no `git.*` Tool ... no workflow can reach this service yet" sentence above no longer holds.** Real `platform.git.commit`/`platform.git.push` tools exist and are ToolInvoker-reachable; a real `git-push` agent chains them as `se.delivery_pipeline`'s final step; and (`P03-S01-M24-T02`) `bootstrap.py`'s real HTTP-route composition now threads a real, `AIOS_GIT_*`-configured `GitIntegrationService` when one is configured (still `None`/the safe no-op in every environment today). The protected-branch structural refusal named above is exactly what keeps T11 mitigated once reachability stops being latent — the exposure T11 names is no longer absent-by-omission, it is absent-by-the-service's-own-structural-refusal, which is the stronger, intended state.
- **Updated (2026-08-02, `P03-S01-M24-T03`): reachability itself is now also permission-gated, not merely structurally narrow.** Before this step, once a real `git_service` was configured anywhere in the registry, *every* `sandbox:execute` agent in the pack — not only `git-push` — could reach `platform.git.push`, since `build_pack_context` gated Git-tool injection on `sandbox:execute` alone. Closed: it now also requires the entrypoint's own declared `git:write` (see T5's own new bullet above), so a compromised or misconfigured non-`git-push` agent genuinely cannot reach a Git write tool even when the service is fully configured.
- **Correcting line 34's own T11 claim (`P03-S06-M41-T02`, 2026-08-06): "no Git Integration Service exists" is stale by the same `P03-S01-M24-T02`/`T03` steps line 37 already flags — but `tests/security/test_t11_git_safety.py` still only tests the manifest-schema layer, not a positive-control proof.** A real `GitIntegrationService`/protected-branch policy now exists and is proven end to end in `test_delivery_pipeline_git_push.py` (a real commit and push, and — separately — a real, structural refusal path via `build_pack_context`'s `git:write` gating above). No test anywhere yet drives a live, configured `GitIntegrationService` through a genuine force-push/branch-delete *attempt* and asserts its own `GitPushPolicy` refuses it — T11's positive-control gap `line 34` names for T11 specifically is therefore still real, just for a narrower reason than "no service exists." A real, disclosed, correctly-sized candidate for its own future ticket, not attempted here.
...
**Manifest signing moved into v1 scope on 2026-08-12 (product-owner decision) and is now [FR-117](../02_requirements/functional/functional_requirements.md).** This paragraph previously read: "Manifest signing is not implemented in v1. This is a known gap... until signing exists, pack provenance is enforced by controlling the install path (packs come from the repository or an internal index, not arbitrary URLs) and by human-approved activation." Those two controls remain real and remain in force; signature verification is being added **in addition to** them, not as a replacement. **Built the same day (`P01-S03-M28-T02`).** Three decisions were put to the product owner rather than guessed, because no document had ever specified any of them:

- **Scheme — Ed25519 detached signatures.** Asymmetric, so a verifying node holds only public keys and cannot forge what it can check; an HMAC shared secret would have given integrity but not authorship, which is the whole point of provenance. No new dependency: `cryptography` is already present via `pyjwt[crypto]`. Sigstore was considered and rejected — it needs network reachability and OIDC infrastructure at verify time, against this platform's deny-by-default egress posture.
- **Trust anchor — a directory of PEM public keys**, path from `PlatformConfig.manifest_trust_store_dir`, the same shape as `capability_pack_dirs`. A public key is not a secret, so anchors are committed and reviewed in git and rotation is a visible diff. The Secrets Manager was rejected for that reason: it would emit secret-access audit rows for material that is safe to publish.
- **Posture — config-gated, default off.** `require_signed_manifests` defaults to `false`, so all three existing unsigned packs load exactly as before; set it `true` and anything not `signed_and_valid` is refused at load, before installation or activation — FR-001's "reject without partial registration" shape.

**What is signed:** the SHA-256 of the manifest's canonical JSON (reusing `canonical_json_sha256`, the hashing primitive this codebase's tamper-evident writers already share), not the raw file bytes — a reformatted manifest, or one checked out with different line endings, is the same manifest. The signature lives beside it as `<manifest>.sig`. The schema is unchanged: an in-manifest field would be self-referential and would alter the contract every existing pack validates against.

**Honest guarantees, not assumed ones.** Verification always runs and always reports, even with enforcement off, so the signing state of the estate is observable before anyone turns enforcement on. Four outcomes, not two — `signed_and_valid`, `unsigned`, `invalid`, and `unverifiable` (a signature is present but no trust anchor exists to check it). `unverifiable` is refused as firmly as `invalid` when enforcement is on: absence of proof is not proof, and collapsing it into `unsigned` would make a misconfigured deployment look like a clean one. This mirrors `SandboxGuarantees`' own "report what is actually enforced" pattern.

**Still true, and deliberately unchanged:** signing is *added to* the two pre-existing controls, not a replacement for them. Install-path control and human-approved activation both remain in force. No private key exists in this repository and none is needed — only public anchors are ever deployed.
...
This document establishes the security architecture and threat model for v1. Known accepted gaps, as originally recorded: manifest signing (§8), multi-tenant isolation (§4), and gVisor as a non-default sandbox runtime (§5.1). **Manifest signing left that list on 2026-08-12** — a product-owner decision moved it into v1 scope as FR-117; it is now required work, not an accepted gap, and §8 records that the requirement exists while the design does not yet. Each is recorded in the Decision Log's open decision points with a trigger for revisiting.
