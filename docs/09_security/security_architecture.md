...
- **Updated (2026-08-01, `P03-S05-M14-T03`): §9's monotonic-narrowing intersection (T5's primary control) is now real and tested, but has no real data to run against yet.** `security_manager.narrowing.narrow_permissions` computes the exact principal ∩ workflow ∩ agent ∩ tool formula and is proven against the real security property. Still missing: no code reads a workflow's/agent's/tool's declared permissions out of a manifest at authorization time, so nothing calls this computation during a real invocation yet. See `authentication_authorization.md`'s own Implementation Status for the full detail. T5 is therefore still only partially mitigated, not resolved as §4's table implies — the gap moved from "no computation" to "no data source," not closed.
