...
## Implementation Status (2026-08-03, `P03-S05-M14-T09`; `workflow:control` added 2026-08-10, `P06-S01-M36-T04`; `knowledge:read` added 2026-08-14, `P02-S04-M09-T08`)

**`knowledge:read` is now modelled and enforced (`P02-S04-M09-T08`), and it is granted to *every* role — a deliberate, documented departure from the "nothing documented for this role, no grant" discipline every other permission here follows.** §4.2's role table says nothing about knowledge at all, so that discipline offers no answer; knowledge is the platform's own approved documentation (`../03_architecture/kernel/knowledge_manager.md` §3 puts its authority at "Highest", §6 calls repository documentation "the primary source of truth") and no document anywhere ranks reading it against workflow or evaluation reads or suggests it is privileged. Product-owner decision, 2026-08-14. The permission string itself was **not invented**: `knowledge:read`/`knowledge:write` have been in `platform_sdk/schemas/manifest.schema.json` — the closed vocabulary ADR-0023 requires be published in the SDK — since that schema was written; only `knowledge:read` gains a Kernel constant, because nothing writes knowledge on behalf of a principal (ingestion is a platform startup scan with no principal), and a permission with no enforcement point is exactly what this module's own discipline exists to prevent. Enforced as a **SQL predicate** in `knowledge_manager.query_engine`, never a post-filter. **The gate fails closed (R-021, 2026-08-14): a request carrying no principal is denied**, in line with ADR-0023's "absence of a permission is denial, never a default-allow". It was briefly fail-open, until an investigation found a real authenticated bypass — `ExperimentRunOrchestrator` took a principal's id but not their permissions, so experiment-run instances persisted `principal_permissions = NULL` and retrieved knowledge ungated (not exploitable as shipped: every role holding `experiment:run` also holds `knowledge:read`). Both the bypass and the fail-open default are fixed, on the reasoning that a permissive default leaves every future creation path one forgotten argument from silently disabling the control, whereas denying wrongly costs only a thinner prompt.
