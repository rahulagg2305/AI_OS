...
**§4.4's "Monotonic Narrowing — the core rule" is partially implemented: the computation is real, its data source is not.** `ai_os_kernel.security_manager.narrowing.narrow_permissions`/`is_permitted` (`P03-S05-M14-T03`) compute ADR-0023's exact intersection (principal ∩ workflow ∩ agent ∩ tool) and are proven against the real security property (a broad principal genuinely narrowed by restrictive declarations; no hop can manufacture a permission the principal lacks). What is still missing: no code anywhere parses a workflow's, agent's, or tool's declared `permissions` out of a manifest into a runtime value, so nothing yet calls this computation during a real invocation — that remains Capability Manager territory. `SecurityContext.permissions` still reflects only the principal term today, for that reason. §4.4's "There is no runtime elevation path" is true today only in the narrow sense that nothing elevates a principal's own permission set; the fuller per-step narrowing the intersection implies does not yet run against real data.
