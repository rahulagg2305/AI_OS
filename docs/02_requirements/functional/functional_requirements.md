...
**All 76 FRs are classified below.** Counts: **61 MUST** (v1), **14 SHOULD** (v1 if capacity allows), **1 LATER** (FR-044, post-v1). **Corrected 2026-08-13 during the R-006 recount** — this line said "75 FRs / 60 MUST" and was stale by exactly one row: **FR-117** (signed manifests, Stage A) was added 2026-08-12 and never counted here. Recounted directly from this document's own tables, not carried forward.
...
| "all of §6 (FR-070–FR-078, benchmarking)" not built | **False** | Seven of the nine have real code: FR-070 (`evaluation_engine/experiment_repository.py`), FR-071 (per-run model pinning — `experiment_run_orchestrator.pin_definition_to_model`, so the same definition genuinely runs against each declared model with only that dimension varied), FR-072 (`capability_packs/benchmarking/.../replicate_management.py`), FR-073 (`evaluation_engine/metrics_collector.py`), FR-074/FR-075 (`evaluation_engine/comparison_computer.py`), FR-076 (`capability_packs/benchmarking/.../cost_ceiling.py`), FR-077 (`capability_packs/benchmarking/.../prompt_adaptation.py`) — **paths corrected 2026-08-13 during the R-006 recount: three of these were cited under `evaluation_engine/`, where they do not exist; the code is real, the paths were wrong**. The full define→run→compare loop is reachable over HTTP (§6.3 of `../../07_api/api_architecture.md`). Only **FR-078** (historical trend view) is genuinely unbuilt. |
