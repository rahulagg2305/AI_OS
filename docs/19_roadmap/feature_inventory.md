...
**A second, distinct known regression, found at step 12: migrated agents that write files through the sandbox can no longer dynamically discover the correct Python interpreter command for the active sandbox backend.**
...
- **Resolution:** `python_command` is now a `BuildAgentEntrypoint` constructor default (`("python3",)`, matching `DockerSandbox`, the real, documented system-wide default backend — `AIOS_SANDBOX_BACKEND` defaults to `"docker"`), overridable by a caller that knows better (every test in this pack now does, matching whichever sandbox it actually constructs).
- **The real, narrow regression risk:** if a real deployment ever sets `AIOS_SANDBOX_BACKEND=local` without also reconfiguring this agent's own default, the write step fails with a real, clear `platform.sandbox.run_command` execution error (the interpreter named by the stale default is not found) rather than silently writing nothing — a loud failure, not a silent wrong write, but a real coupling the old code's own dynamic lookup never had.
- **Affects:** `build` (step 12) today; will affect `documentation` (step 13) too, if that agent's own migration needs the same write path (to be confirmed when that step starts).
- **What closes this gap:** a future SDK extension letting `ToolInvoker`/`ToolDescriptor` report backend-specific facts like the portable interpreter command, or a real per-workflow/per-step configuration mechanism a caller could use to supply it declaratively — both out of scope for v1.0.0. Until then, any operator changing `AIOS_SANDBOX_BACKEND` away from its default must also update this agent's own `python_command` default (or however documentation.py's own migration will need it), a manual coupling now recorded here rather than left to be silently rediscovered.
- **First recorded:** step 12 (`platform_sdk_v1_scope.md` §6o).
