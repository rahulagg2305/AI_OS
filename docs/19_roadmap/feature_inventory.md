...
- **Affected so far:** `requirements-analyst` (step 10) and `architecture` (step 11) — both make real LLM completions and are both migrated. **`qa-test` (step 9) is unaffected** — it makes no LLM call at all (sandbox-only agent), so there was nothing to stop recording.
- **Will affect, once migrated:** `build` (step 12) and `documentation` (step 13) — both are `PromptedAgent`-backed and both currently have their own real completions recorded exactly as `requirements-analyst`/`architecture` used to. Migrating either reproduces this identical gap.
...
- **First recorded:** step 10 (`platform_sdk_v1_scope.md` §6m); reconfirmed as a second, not a one-off, occurrence at step 11 (§6n); tracked here from step 11 onward specifically so it survives past individual step reports and gets picked up as a real follow-up once all remaining agents are migrated.
