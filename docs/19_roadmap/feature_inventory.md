...
**Created:** 2026-07-28
**Last Updated:** 2026-07-28 (module 27 row updated: Platform SDK v1.0.0 is now scoped as a concrete, evidence-based 15-step plan — see `../03_architecture/platform/platform_sdk_v1_scope.md`, awaiting approval, no code yet. Prior entry: product-owner decision recorded — module 27, Platform SDK, is a **hard gate** blocking any new agent (module 29) or new pack (modules 32–34) until it is built. Completed the documentation audit's remaining ~20 files. See `history/026_platform_sdk_gate_and_audit_completion.md`.)
...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 5% | schema-only | **🛑 HARD GATE (product-owner decision, 2026-07-28): no new agent may be added to any Capability Pack, and no new Capability Pack may be added, until this module is built.** `contracts/`, `models/`, `sdk/`, `utilities/`, `prompts/` directories are all empty (`errors/`, `testing/` don't exist at all). Packs currently import Kernel internals directly as a documented, temporary compromise (`capability_pack_contract.md`'s dated exception note, now bounded by this gate). The five existing SE pack agents (module 29) are grandfathered and unaffected. **Scoped 2026-07-28**: `../03_architecture/platform/platform_sdk_v1_scope.md` is a 15-step, evidence-based build plan (5 of 15 Protocols needed by real usage; migration + `pack_contract_suite` sequencing decided) — awaiting product-owner approval before step 1 starts. The gate lifts only once the plan's compliance-completion step (15) lands, not at SDK-scaffold alone. |
...
**Overall weighted completion: ≈ 21%** (modules 20/29/41 all moved up slightly this step — real Docker verification landed, and a fifth Software Engineering agent, Requirements Analyst, was built; see `history/024_docker_verification_and_requirements_analyst.md`).
