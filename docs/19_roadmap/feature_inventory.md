...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 15% | error taxonomy + shared models; no Protocols yet | **🛑 HARD GATE (product-owner decision, 2026-07-28) still active — an SDK existing does not lift it; only `platform_sdk_v1_scope.md` step 15 does.** **Sequence revised 15 → 18 steps (2026-07-29)** after an independent architecture review found 3 blocking gaps: no step made the Kernel *supply* the Protocols, the injection path (`EntrypointLoader` hardcodes `cls()`; `PackContext` is identity-only) was a non-goal, and the specified `Agent`/`Tool` shapes are incompatible with `SqlAgentRegistry`'s `isinstance` gate. New steps 2a/6a/6b added; `SecretResolver` dropped; migration reordered `qa-test`-first; `pipeline.py` relocation is now step 7. See §6a. **Steps 1–2 done:** real installable distribution; `ai_os_sdk.errors` (6-category `ErrorCategory`, `StructuredError`, `AiOsError` + 6 subclasses) and `ai_os_sdk.models` (`ArtifactRef`, canonical `TraceContext`, `SecurityContext`, `StepBudget`) are real. **No Protocol exists yet; nothing consumes the package.** Next: step 2a, the Protocol/reality reconciliation decision (docs only). |
