...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 30% | taxonomy + models + all 5 shapes decided + 4 of ~9 Protocols | **🛑 HARD GATE (2026-07-28) still active — only `platform_sdk_v1_scope.md` step 15 lifts it.** 18-step sequence (revised from 15 after an architecture review found 3 blocking gaps — §6a). **Steps 1, 2, 2a, 3, 4, 5 done:** real installable distribution with `py.typed`; `ai_os_sdk.errors` + `ai_os_sdk.models` (error taxonomy, `ArtifactRef`, canonical `TraceContext`, `SecurityContext`, `StepBudget`, the `LLMGateway` model set at the real 13-field `ProviderCapabilities` shape, and `RenderedPrompt`); `ai_os_sdk.contracts` holds `Agent`/`Tool`/`TrustTier`, `LLMGateway`, and now `PromptRegistry` (documented keyword call style kept over the Kernel's own request-object envelope — the one reversal decision from 2a). **Structural compatibility proven against all 5 real pack agents, both real tools, and the real `DispatchingLLMGateway`; `PromptRegistry`'s conversion to/from the real Kernel prompt models proven lossless instead (no real class implements this from-scratch shape). Zero Kernel/pack source modified** (§6c, §6d, §6e). **Still nothing consumes the package.** Next: step 6, `ToolInvoker` (from-scratch design grounded in a new `platform.sandbox.run_command` tool id). |
