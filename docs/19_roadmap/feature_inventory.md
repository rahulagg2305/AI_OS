...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 50% | taxonomy + models + all 5 shapes decided + all Protocols genuinely implemented + injection mechanism proven + entry-point contract formally landed | **🛑 HARD GATE (2026-07-28) still active — only `platform_sdk_v1_scope.md` step 15 lifts it.** 18-step sequence (revised from 15 after an architecture review found 3 blocking gaps — §6a). **Steps 1, 2, 2a, 3, 4, 5, 6, 6a, 6b, 7 done:** real installable distribution with `py.typed`; `ai_os_sdk.errors` + `ai_os_sdk.models` (error taxonomy, `ArtifactRef`, canonical `TraceContext`, `SecurityContext`, `StepBudget`, the `LLMGateway` model set, `RenderedPrompt`, `ToolDescriptor`/`ToolStatus`/`ToolResult`, and now the `ContextService` boundary models); `ai_os_sdk.contracts` holds `Agent`/`Tool`/`TrustTier`, `LLMGateway`, `PromptRegistry`, `ToolInvoker`, `PackContextReceiver`, `ContextService`, and now `CapabilityPack`/`PackContext`/`PackRegistration`/`HealthReport`. **Structural compatibility proven against all 5 real pack agents, both real tools, the real `DispatchingLLMGateway`, and now the real `DefaultContextManager`; `PromptRegistry`/`ToolInvoker` proven by conversion/execution instead of `isinstance` (from-scratch shapes).** **Step 6a** built real Kernel-side adapters genuinely implementing all three permission-gated Protocols. **Step 6b** resolved the `EntrypointLoader` injection blocker via `PackContextReceiver`, without changing `EntrypointLoader`. **Step 7**: `pipeline.py` relocated out of the pack's shipped wheel into `tests/integration/_delivery_pipeline.py` (proven via a real wheel build — the file and its `sqlalchemy` import both confirmed absent — and both relocated integration tests, including the real Docker-backed one, passing unchanged); `ContextService` boundary models landed and narrowed against the real Kernel shape; the full entry-point contract relocated from the Kernel into `ai_os_sdk.contracts.capability_pack`, additively, with the Kernel's own module now a real re-export — real pack source needed zero changes. No blocker found at any point. **Still nothing wires the injection mechanism into `bootstrap.py`/`SqlAgentRegistry`/any real pack** — that's steps 9–14's own migration work; step 8 (check 7 + waiver) is next. Zero pack source modified (§6c–§6i). |
