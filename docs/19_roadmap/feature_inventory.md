...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 10% | packaging scaffold only | **🛑 HARD GATE (product-owner decision, 2026-07-28) still active — an SDK existing does not lift it; only `platform_sdk_v1_scope.md` step 15 does.** **Step 1 of 15 complete (2026-07-28)**: `platform_sdk/` is now a real, installable `ai-os-sdk` PEP 621 distribution (`import ai_os_sdk` works), a workspace member, with 6 stub subpackages (`contracts/`, `models/`, `errors/`, `sdk/`, `utilities/`, `testing/` — the latter two created fresh, did not exist before). **No Protocol, model, or error class exists yet; nothing consumes this package.** `prompts/` stays empty and undecided (see the scope document §5). The five existing SE pack agents (module 29) are grandfathered and unaffected until their own migration steps (9–13). Next: step 2, the `AiOsError` hierarchy + shared boundary models. |
...
**Overall weighted completion: ≈ 21%** (module 27, Platform SDK, moved 5% → 10% this step — packaging scaffold only, not enough on its own to move the rounded overall figure; see `../03_architecture/platform/platform_sdk_v1_scope.md` step 1. Prior movement: modules 20/29/41 moved up when real Docker verification landed and a fifth Software Engineering agent, Requirements Analyst, was built; see `history/024_docker_verification_and_requirements_analyst.md`).
