...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 55% | taxonomy + models + all 5 shapes decided + all Protocols genuinely implemented + injection mechanism proven + entry-point contract formally landed + check 7 real and CI-wired | **🛑 HARD GATE (2026-07-28) still active — only `platform_sdk_v1_scope.md` step 15 lifts it.** 18-step sequence (revised from 15 after an architecture review found 3 blocking gaps — §6a). **Steps 1, 2, 2a, 3, 4, 5, 6, 6a, 6b, 7, 8 done:** real installable distribution with `py.typed`; `ai_os_sdk.errors` + `ai_os_sdk.models` (error taxonomy, `ArtifactRef`, canonical `TraceContext`, `SecurityContext`, `StepBudget`, the `LLMGateway` model set, `RenderedPrompt`, `ToolDescriptor`/`ToolStatus`/`ToolResult`, and the `ContextService` boundary models); `ai_os_sdk.contracts` holds `Agent`/`Tool`/`TrustTier`, `LLMGateway`, `PromptRegistry`, `ToolInvoker`, `PackContextReceiver`, `ContextService`, `CapabilityPack`/`PackContext`/`PackRegistration`/`HealthReport`; `ai_os_sdk.testing` now holds `forbidden_imports`/`waiver` (check 7 + its expiring waiver mechanism). **Structural compatibility proven against all 5 real pack agents, both real tools, the real `DispatchingLLMGateway`, and the real `DefaultContextManager`; `PromptRegistry`/`ToolInvoker` proven by conversion/execution instead of `isinstance` (from-scratch shapes).** **Step 6a** built real Kernel-side adapters genuinely implementing all three permission-gated Protocols. **Step 6b** resolved the `EntrypointLoader` injection blocker via `PackContextReceiver`. **Step 7**: `pipeline.py` relocated out of the pack's shipped wheel; `ContextService` landed; the entry-point contract relocated into `ai_os_sdk.contracts.capability_pack`, additively. **Step 8**: check 7 (`ai_os_sdk.testing.forbidden_imports`, a real AST scanner) plus its documented, expiring waiver (`ai_os_sdk.testing.waiver`) are real — found 44 real `ai_os_kernel` imports across 6 real modules in this pack, proven to genuinely fail without a waiver and genuinely pass with one (automated regression test, not a one-off run), and wired into CI via `scripts/check_import_boundaries.py` landing in a pre-existing gate. No blocker found at any point. **Still nothing wires the injection mechanism into `bootstrap.py`/`SqlAgentRegistry`/any real pack** — that's steps 9–14's own migration work, `qa-test` next. Zero pack source modified (§6c–§6j). |
