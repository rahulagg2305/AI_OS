...
| 29 | SE Pack — Agents | C | 25% | minimal slice | 4 of 16 documented agents real (`architecture`, `build`, `qa-test`, `documentation`); 12 remain, including `requirements-analyst`/`technical-planner`/`code-reviewer` named explicitly in Stage C's own exit criteria. |
