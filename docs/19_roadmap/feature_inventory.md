...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 26% | taxonomy + models + all 5 shapes decided + 3 of ~9 Protocols | **🛑 HARD GATE (2026-07-28) still active — only `platform_sdk_v1_scope.md` step 15 lifts it.** 18-step sequence (revised from 15 after an architecture review found 3 blocking gaps — §6a). **Steps 1, 2, 2a, 3, 4 done:** real installable distribution with `py.typed`; `ai_os_sdk.errors` + `ai_os_sdk.models` (error taxonomy, `ArtifactRef`, canonical `TraceContext`, `SecurityContext`, `StepBudget`, plus the `LLMGateway` model set: `LLMRequest`, `LLMResponse`, `UsageRecord`, `ProviderCapabilities` at the real 13-field shape); `ai_os_sdk.contracts` holds `Agent`/`Tool`/`TrustTier` and now `LLMGateway`, narrowed to `complete()`/`capabilities()`. **Structural compatibility proven against all 5 real pack agents, both real tools, and the real `DispatchingLLMGateway`, zero Kernel/pack source modified** (§6c, §6d). **Still nothing consumes the package.** Next: step 5, `PromptRegistry` (documented keyword call style kept; `version` required). |
