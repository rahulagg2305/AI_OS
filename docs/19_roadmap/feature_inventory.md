...
**Migrating an agent onto the Platform SDK silently drops its real LLM completions from `evaluation.llm_calls` (module 12, Evaluation Engine). This is a genuine, real observability regression, not a documentation nit, and it compounds with every further migration step.**
...
- **Affected so far:** `requirements-analyst` (step 10), `architecture` (step 11), and `build` (step 12) — all three make real LLM completions and are all migrated. **`qa-test` (step 9) is unaffected** — it makes no LLM call at all (sandbox-only agent), so there was nothing to stop recording.
- **Will affect, once migrated:** `documentation` (step 13) — the last `PromptedAgent`-backed agent, currently still recording its own real completions exactly as the other three used to. Migrating it reproduces this identical gap a fourth time, closing out the full set of LLM-calling agents.
- **What closes this gap:** a future Platform SDK version adding a real `Telemetry`/call-recording surface a pack can depend on without reaching into Kernel internals — explicitly out of scope for v1.0.0 (`platform_sdk.md` §2's own deferral list). Until then, **`evaluation.llm_calls` will under-count real completions for every migrated agent**, growing to all four LLM-calling agents (`requirements-analyst`, `architecture`, `build`, `documentation`) by the time step 13 completes. Any future consumer of `evaluation.llm_calls` (cost dashboards, usage audits, the Evaluation Engine itself once built) must know this before trusting that table's totals as complete.
- **First recorded:** step 10 (`platform_sdk_v1_scope.md` §6m); reconfirmed as a second occurrence at step 11 (§6n), and a third at step 12 (§6o); tracked here from step 11 onward specifically so it survives past individual step reports and gets picked up as a real follow-up once all remaining agents are migrated.
