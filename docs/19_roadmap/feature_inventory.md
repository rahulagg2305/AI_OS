...
| 27 | Platform SDK (`ai-os-sdk` package) | B | 58% | taxonomy + models + all 5 shapes decided + all Protocols genuinely implemented + injection mechanism proven + entry-point contract formally landed + check 7 real and CI-wired + first real agent migrated | **🛑 HARD GATE (2026-07-28) still active — only `platform_sdk_v1_scope.md` step 15 lifts it.** 18-step sequence (revised from 15 after an architecture review found 3 blocking gaps — §6a). **Steps 1, 2, 2a, 3, 4, 5, 6, 6a, 6b, 7, 8, 9 done:** real installable distribution with `py.typed`; `ai_os_sdk.errors` + `ai_os_sdk.models` (error taxonomy, `ArtifactRef`, canonical `TraceContext`, `SecurityContext`, `StepBudget`, the `LLMGateway` model set, `RenderedPrompt`, `ToolDescriptor`/`ToolStatus`/`ToolResult`, and the `ContextService` boundary models); `ai_os_sdk.contracts` holds `Agent`/`Tool`/`TrustTier`, `LLMGateway`, `PromptRegistry`, `ToolInvoker`, `PackContextReceiver`, `ContextService`, `CapabilityPack`/`PackContext`/`PackRegistration`/`HealthReport`; `ai_os_sdk.testing` holds `forbidden_imports`/`waiver` (check 7 + its expiring waiver mechanism). **Structural compatibility proven against all 5 real pack agents, both real tools, the real `DispatchingLLMGateway`, and the real `DefaultContextManager`.** **Step 6a** built real Kernel-side adapters. **Step 6b** resolved the `EntrypointLoader` injection blocker via `PackContextReceiver`. **Step 7**: `pipeline.py` relocated; `ContextService` landed; the entry-point contract relocated into `ai_os_sdk.contracts.capability_pack`. **Step 8**: check 7 + its expiring waiver are real and CI-wired. **Step 9 — the first real migration**: `qa-test` now depends only on `ai_os_sdk` (`Agent`+`PackContextReceiver`, sandbox via injected `ToolInvoker`). Two real gaps found and resolved: the real Workflow Engine context object is still Kernel-typed (duck-typed instead of `isinstance`-checked); `SqlAgentRegistry` has no real `bind_pack_context()` wiring (a real, unconditionally-run integration test does it itself, standing in for still-missing production wiring — recorded as an open item for a future step, not silently left undiscovered). Zero behavior change proven via both real Delivery Pipeline end-to-end tests (deterministic + live Docker) producing identical output. `qa-test` removed from the waiver (6→5 modules, 44→40 violations); check 7 now genuinely passes for it, unwaived. No blocker in the injection mechanism's own design. Zero pack source modified beyond the one agent being migrated (§6c–§6k). |
