...
| R-021 | Knowledge retrieval is permission-gated only where a principal is actually threaded. `P02-S04-M09-T08` made `knowledge:read` a real SQL predicate in `QueryEngine`, and `AgentStepExecutor` now carries `principal_permissions` into `ContextRequest` — but **`None` means unenforced, not denied**, and most retrieval paths still carry no identity at all. ADR-0023 says "absence of a permission is denial, never a default-allow", so the literal reading is stricter than what is built. Two further limits: the gate is **binary** (`knowledge.documents` has no owner or classification column, so a permitted principal sees every non-archived document), and `project_id` scoping — required by `knowledge/knowledge_base_structure.md` §3 to stop one project's content reaching another's context — is **not implementable today** because `workflow_instances` has no `project_id`, nothing in the Workflow Engine carries one, and ingestion writes `NULL` for every document | M | **Open — deliberately, by product-owner decision 2026-08-14.** Denying on absent identity would silently stop `se.delivery_pipeline`'s already-running `requirements-analyst` step from retrieving anything: a behaviour change to working code, not a new control. An empty permission set **is** denied, so the control genuinely bites on a known-but-unpermissioned principal. Closes in two parts: thread a principal through every retrieval path and flip the default to deny; then establish a real principal-or-workflow-to-project binding so scoping can replace the binary gate | Access/Filter Layer step, 2026-08-14 |
