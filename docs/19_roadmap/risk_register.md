...
| R-021 | Knowledge retrieval was permission-gated only where a principal was actually threaded: `P02-S04-M09-T08` made `knowledge:read` a real SQL predicate in `QueryEngine`, but **`None` meant unenforced, not denied**, so any path that failed to carry identity bypassed the control silently. Remaining limit: the gate is **binary** (`knowledge.documents` has no owner or classification column, so a permitted principal sees every non-archived document), and `project_id` scoping — required by `knowledge/knowledge_base_structure.md` §3 to stop one project's content reaching another's context — is **not implementable today** because `workflow_instances` has no `project_id`, nothing in the Workflow Engine carries one, and ingestion writes `NULL` for every document | M | **Fail-open half CLOSED 2026-08-14; binary/scoping half open.** A targeted investigation disproved the premise the original decision rested on and found a **real, authenticated bypass**: `ExperimentRunOrchestrator.run` took the principal's *id* but not their permissions (`routes/experiments.py` authenticates with `require_permission(EXPERIMENT_RUN)`, so a real `SecurityContext` existed and was dropped one call short), leaving every instance created by `POST /experiments/{id}/run` with `principal_permissions = NULL` and retrieving knowledge with the gate off. **Not exploitable as shipped** — every role holding `experiment:run` (operator/maintainer/admin) also holds `knowledge:read`, verified programmatically — but one role-table edit from being live. Fixed at source, **and the gate now fails closed**: `None` denies. Affordable because the cost of denying wrongly is a thinner prompt, not a failed workflow — `KnowledgeResolver` simply contributes no items. Proven end to end through the real production wiring: the identical trigger, content and resolver, differing only in identity, yields the knowledge or withholds it. **Still open:** replacing the binary gate with real `project_id` scoping, which needs a principal-or-workflow-to-project binding that does not exist | Access/Filter Layer step, 2026-08-14; fail-closed 2026-08-14 |
