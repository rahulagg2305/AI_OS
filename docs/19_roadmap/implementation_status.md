...
**Step 5 of 18 — the `PromptRegistry` Protocol and its models**, at the shape decided in step 2a: **kept** at the documented pack-facing keyword call style (`render(prompt_id, variables, *, version)`) rather than the Kernel's request-object envelope — the one deliberate reversal in the specification's favour, since a pack-facing Protocol should be shaped for its caller, not its implementer (ADR-0004). `version` is **narrowed to required** (nothing resolves a default; relaxing later is backward-compatible, the reverse is not). `get()` and two `RenderedPrompt` fields (`variables_used`, `cache_boundary_index`) stay deferred — no implementation exists at any layer.
