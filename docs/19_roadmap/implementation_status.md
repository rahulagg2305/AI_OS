...
**Step 6 of 18 — the `ToolInvoker` Protocol, `ToolDescriptor`, `ToolResult`, and the `platform.sandbox.run_command` tool contract**, at the from-scratch design decided in step 2a: the documented `invoke(tool_id, inputs, timeout) -> ToolResult` signature kept, grounded in a platform-provided tool id (since the one real pack declares zero tools and `SandboxedCommandTool.execute(inputs)` ignores its own `inputs` entirely). `ToolResult` moves here from step 3 — narrowed (`stdout_ref`/`stderr_ref` → inline strings, `artifacts` deferred, both for lack of a `StorageService`) and extended (`timed_out`/`truncated`, two real outcomes the original spec cannot express). `SecretResolver` remains dropped.

Full 18-step sequence, current position: **Steps 1, 2, 2a done.** Then 3 → 4 → 5 → 6 → 6a → 6b → 7 → 8 (SDK build + Kernel adapters + injection path + check 7) → 9–14 (migration, `qa-test` first) → 15 (compliance completion, and only then does the growth gate lift). See `../03_architecture/platform/platform_sdk_v1_scope.md`; each step is individually approved before it starts.
