...
- **No Protocol code exists yet.** Step 3 is the first to write one, and its scope is now *smaller* than originally planned: `Agent` + `Tool` Protocols only, since `AgentRequest`/`AgentResult`/`ToolRequest` have no consumer under the narrowed shapes and are deferred past v1.0.0, while `ToolResult` moves to step 6 where its consumer lives.
...
**Step 3 of 18 — the `Agent` and `Tool` Protocols, at the narrowed shapes decided in step 2a.** Two `Protocol` definitions in `platform_sdk/src/ai_os_sdk/contracts/{agent,tool}.py`, structurally identical to the working Kernel shapes (`output_schema: dict` + `async execute(inputs: dict) -> dict`, plus `trust_tier` on `Tool`) so that every one of the five real agents already satisfies them without modification. **Deliberately no request/result models** — `AgentRequest`/`AgentResult`/`ToolRequest` are deferred past v1.0.0, and `ToolResult` belongs to step 6 with its consumer. Smaller than originally scoped; see `platform_sdk_v1_scope.md` §6b.
