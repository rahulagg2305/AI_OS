...
**Step 4 of 18 — the `LLMGateway` Protocol and its model set**, at the shape decided in step 2a: **narrowed** to the two methods that actually exist (`complete()`, `capabilities()`), with `stream()`/`embed()`/`count_tokens()` deferred until a provider adapter implements any of them; and `ProviderCapabilities` **extended** to the real 13 fields, since the working `StaticCapabilityNegotiator` already carries 13 and its own docstring names `platform_sdk.md` as the discrepancy it "implements past." Also lands `LLMRequest`/`LLMResponse`/`UsageRecord`. **Unblocked:** it depends only on step 2's `StructuredError`/`TraceContext` and the step-2a decision, both complete, and has no dependency on step 3's Protocols.
