...
**Updated 2026-08-08 — this section was severely stale, predating essentially all of Stage G's real work.** Real, done: OIDC authentication (`P07-S02-M14-T01`) and the full five-role model (`P07-S02-M14-T02`, persisted grants now honored by every permission-checked route); the Vault secrets backend and the real `AIOS_SECRET_BACKEND` switch (`P07-S02-M19-T01`); a real, partial Kubernetes/Helm chart proven against a live `kind` cluster (`P07-S01-M40-T01`); a real, opt-in NetworkPolicy egress allowlist (`P07-S01-M40-T02`); real backup/restore tooling with a real, automated rehearsal proving the audit hash chain survives a restore (`P07-S01-M40-T03` — the "workflow resumption" half of the exit criterion below remains a disclosed, unbuilt gap); `tests/performance/` is real, 10 repeatable tests (see `nfr.md` §13); chaos tests exist for 2 of 3 named scenarios (`P07-S03-M42-T01`: kill worker, drop provider — both real; kill Redis is genuinely blocked, not deferred, since no real Kernel consumer of Redis exists anywhere to chaos-test). Still ⬜: cost anomaly alerting; coverage thresholds enforced in CI (`pytest-cov` configured, no threshold gated); the gVisor/Firecracker sandbox pattern ADR-0020 itself calls "deliberate node configuration"; T1–T12 threat-control test coverage (`tests/security/` exists but has not been audited against the full T1–T12 list as part of this update).
...
**Exit status: in progress, not complete.** Most named deliverables above are now real; the workflow-resumption half of the restore rehearsal, cost anomaly alerting, coverage gating, the stronger sandbox tier, and a full T1–T12 audit remain open.
