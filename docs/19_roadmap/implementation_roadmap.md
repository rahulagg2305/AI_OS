...
- ◐ LLM Gateway: one provider adapter, alias routing, retries, fallback, capability matrix, token/cost accounting, prompt-cache planning ([ADR-0002](../18_decision_log/adr/ADR-0002-llm-gateway-single-entry-point.md)) — **two** adapters, routing, retry/fallback, circuit breaker, backoff, capability matrix, and cost accounting all real; **no `stream()`, no `embed()`, no `count_tokens()`, no rate limiter, no prompt-cache planner**
