...
- ◐ LLM Gateway: one provider adapter, alias routing, retries, fallback, capability matrix, token/cost accounting, prompt-cache planning ([ADR-0002](../18_decision_log/adr/ADR-0002-llm-gateway-single-entry-point.md)) — **two** adapters, routing, retry/fallback, circuit breaker, backoff, capability matrix, cost accounting, a real per-provider rate limiter, real `count_tokens()` (Anthropic), and real `embed()` (`local` only — Anthropic's API has none) are all real; **no `stream()`, no prompt-cache planner, no per-principal rate limiting**
