...
**Not yet real: 9** (`database`, `api-designer`, `devops`, `security`, `code-reviewer`, `release`, `refactoring`, `performance`, and `project-intelligence/existing-project-analyzer`) **per this table** — but as this section's own disclosure above notes, `database`/`api-designer`/`security`/`code-reviewer` are genuinely real in the manifest already; this count is only accurate for what remains genuinely unbuilt (`devops`, `release`, `refactoring`, `performance`, `existing-project-analyzer`).
