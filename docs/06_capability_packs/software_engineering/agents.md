...
| `software-engineering/performance` | `ai_os_pack_software_engineering.agents.performance:PerformanceAgentEntrypoint` | Added 2026-08-09 (`P08-S01-M29-T07`, FR-044, `LATER`/post-v1 — the only such requirement in the whole 75-FR backlog, built now because its own ticket was the single remaining ready Task once every higher-priority one was exhausted). "A running system" (this ticket's own literal Input) reduced honestly to one real source file — no real APM/instrumentation infrastructure exists anywhere in this codebase to attach to a genuinely running process. Computes real McCabe cyclomatic complexity per function from the file's own real AST (stdlib `ast`, no LLM call at all — the identical `lint`/`qa-test` shape), reporting real hotspots and mechanically-derived recommendations for any function crossing the real, named threshold of 10. Not chained into any workflow. |

**Not yet real: 2** (`devops`, `project-intelligence/existing-project-analyzer`) **per this table** — `performance` closed 2026-08-09; `database`/`api-designer`/`security`/`code-reviewer`/`release` were already genuinely real in the manifest before this step, per this section's own disclosure above.
...
This section should be updated (or removed, once the gap closes) every time an agent listed above genuinely gets built, per `feature_inventory.md`'s own maintenance discipline.
