...
**Graph — this section's own diagram below is a known-stale snapshot from before Lint, Code Review, Git Push, all three Quality Gates, the routing decision step, and the human approval point existed; `capability_packs/software-engineering/workflows/delivery_pipeline.yaml` and `ai_os_kernel.workflow_engine.delivery_pipeline`'s own module docstring are the real, current, authoritative shape (thirteen declared steps: eight agents, one `decision`, three `quality_gate`, one `human_approval`). Reconciling this diagram in full is a real, separate documentation-reconciliation task, not part of any one step that added a single one of these.** As of 2026-08-06 (`P03-S03-M30-T03`, FR-045's own bounded review-revise loop), `se.delivery_pipeline` **does** include a real `code-review` agent step and its own `quality-gate-code-review-clean` gate (between `quality-gate-lint-clean` and `test`) — the claim below that it has "no ... `code-reviewer`" is no longer accurate. It also includes a real `decision` step (`route-after-build`, between `build` and `lint`) and a real `human_approval` point (`approve-git-push`, between `documentation` and `git-push`, `P03-S03-M30-T05`) — the claim below that it has "no decisions" and none of `se.implement_task`'s step types is no longer accurate for `decision`/`human_approval` specifically (still accurate for `tool`/`parallel`/`foreach`/`sub_workflow`, none of which this pipeline declares).
...
- `se.implement_task`'s own graph includes `fs.apply_patch`/`test.run` tool steps, two `decision` steps forming iteration loops (`max_iterations: 3`), a `code-reviewer` agent step, and a blocking `quality_gate` — `se.delivery_pipeline` now has its own, differently-purposed real `decision` step too (routing, not an iteration loop) and, since `P03-S03-M30-T03`, its own real `code-review` agent step and gate too (a single bounded retry-from-`build` cycle via `retryPolicy`, not `se.implement_task`'s own `max_iterations: 3` decision-loop shape) — but still no `tool` steps or `foreach`.
...
### Real today (`P08-S02-M30-T02`), intentionally reshaped

Declared in `capability_packs/software-engineering/workflows/implement_task.yaml`, composed in `ai_os_kernel.workflow_engine.implement_task`. Real end to end: `implement` (`build`, standing in for the undocumented `backend-developer`) -> `qa-test` -> `tests-passed` (`decision`) -> `code-review` -> `no-blocking-findings` (`decision`) -> `quality-gate-tests-pass`. Three disclosed departures from the graph above, each resolved by reusing something real instead of inventing it — see the YAML's own header comment for the full reasoning: no `backend-developer` agent (no real, data-driven "route by task.kind" mechanism exists either, since `WorkflowStep.agentId` is static per step); no separate `fs.apply_patch`/`test.run` tool steps (`build`/`qa-test` already write/run directly through their own sandbox permission); no per-loop attempt counter (bounded instead by this workflow's own whole-run `max_iterations`, a coarser, honest safety net). **Both `decision` steps genuinely branch backward to `implement`** — real, working, confirmed by direct inspection of `WorkflowDefinition`'s own forward-reference validator and `WorkflowInstanceService._resolve_next_step`, and proven twice against real Postgres, including a genuine loop-back recovery. Not yet chained into any larger workflow — no `foreach` exists to invoke it.
