...
## Implementation Status (updated 2026-08-09, `P05-S02-M32-T03`)

**Built: three real, non-agent Tools feeding this agent's own future scope.** `repository.ingest` (`tools.repository_ingestion`, `tier1_sandboxed`) genuinely walks a real repository and returns a real structural model; `language.detect` (`tools.language_detection`, `tier2_trusted`) consumes that output to detect languages/build systems/frameworks with real confidence per finding; `dependency.graph` (`tools.dependency_graph`, back to `tier1_sandboxed` since it reads and parses real file content) consumes the same structural model, filtered to Python, and constructs a real, queryable module/dependency graph via stdlib `ast` — together, the first three, purely mechanical activities of this document's own §2 "Typical activities: Ingest and structure a codebase" / "Detect languages, frameworks, and platforms" / "Build module and dependency views." Deliberately built as Tools, not an Agent: no LLM call is needed for any of these three mechanical operations (a design fork resolved via `AskUserQuestion` each time). Neither the `existing-project-analyzer` agent itself nor any of the five workflows below exists yet — none of the three Tools is wired into either.
