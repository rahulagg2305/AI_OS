...
## Implementation Status (updated 2026-08-07, `P07-S01-M40-T01`)
...
- **Updated (`P07-S01-M40-T01`): a real, partial Helm chart now exists — `infra/kubernetes/helm/ai-os/`, closing the "no Kubernetes manifests and no Helm chart" gap this line used to name in full.** `Deployment/aios-api` (replicas 2)/`Deployment/aios-worker` (replicas 3), `Service/aios-api` (ClusterIP), `ConfigMap/aios-config`, `ServiceAccount/aios`, `PodDisruptionBudget` (one per role), and `HorizontalPodAutoscaler/aios-worker` are all real — validated via `helm lint`/`helm template` and, further, against a real, temporary `kind` cluster (`kubectl apply --dry-run=server`, then a genuine, non-dry-run apply against a live Kubernetes API server, confirmed via `kubectl get`; pods correctly report `ErrImagePull`, since no container registry has been decided anywhere in this repo — the one real, expected, disclosed limitation, not a manifest defect). Probes on `aios-api` use the real, verified `/api/v1/health/live`/`/api/v1/health/ready` paths this section's own table already names. **Still genuinely unbuilt, each for a real, specific, disclosed reason (see the chart's own README.md): no `Ingress`** (no ingress controller/TLS issuer/hostname decided anywhere); **no `NetworkPolicy` egress allowlist** (no concrete provider IP ranges/FQDNs decided anywhere — inventing them would violate "no hardcoded values" literally); **no `ExternalSecret`** (requires the External Secrets Operator in-cluster, and no Kubernetes secrets backend has been selected — the Vault backend itself, `P07-S02-M19-T01`, is still unbuilt; the chart instead references an already-existing, externally-managed `Secret` by name only); **the HPA scales on CPU, not "worker lease-queue depth"** (that custom metric needs a metrics-adapter — Prometheus Adapter or KEDA — neither built nor installed anywhere); **no Podman-sidecar or gVisor sandbox pattern** (a real, separate, larger piece of work this section's own text already calls "deliberate node configuration"); **`aios-worker` has no HTTP probes at all** (the `worker` entry point runs no HTTP server — verified directly in its own source — so the documented probe paths exist only on the `api` role). `infra/terraform/` remains empty.
...
Consequence: AI_OS now has a real, runnable container image and a real, partial Helm chart proven against a genuine (if temporary, local) Kubernetes cluster — but no real registry, no managed cluster, no Ingress/NetworkPolicy/ExternalSecret, and no release pipeline. Treat this document as the Stage G target, not a description of a fully deployable production system.
