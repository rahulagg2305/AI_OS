...
## Implementation Status (updated 2026-08-03, `P01-S01-M40-T04`)
...
- **The `worker` role remains a genuine, disclosed stub** (`ai_os_kernel.entrypoints.worker:main` logs one line and exits 0) — **correcting this section's own previously-stale claim that "there is no multi-instance worker loop" at all.** `ai_os_kernel.workflow_engine.worker_loop.WorkflowWorkerLoop` is real and already runs continuously (`P02-S01-M05-T14`), but only ever inside the `api` role's own FastAPI lifespan, never as its own long-running process — so `worker`-role horizontal scaling is still untested in any real deployment, for a different reason than previously documented (the loop exists; the *process role* that would run it standalone does not). The container image faithfully reflects this: it runs `worker.py` exactly as it exists, not a Dockerfile bug. Wiring a real, standalone worker process is separate, undone scope.
...
Consequence: AI_OS now has a real, runnable container image, but still runs only as a local development process (or a manually-run container) against Compose-provided Postgres — no orchestration, no staged rollout. Treat this document as the Stage G target, not a description of a fully deployable system.
