...
class TestRealSoftwareEngineeringPackScan:
    ...
    def test_the_exact_two_modules_with_violations(self) -> None:
        ...
        Two modules, not three, as of step 12: qa-test
        (`agents.verification`, step 9), requirements-analyst
        (`agents.requirements_analyst`, step 10), architecture
        (`agents.architecture`, step 11), and build (`agents.build`,
        step 12) are all migrated onto the Platform SDK and no longer
        in this set at all."""
        violations = scan_pack_source(_SE_PACK_SRC, own_pack_package=_SE_PACK_PACKAGE)
    ...
    def test_migrated_agents_are_migrated_and_contribute_zero_violations(
        ...
        """Steps 9, 10, 11, and 12's own real proof, at the scanner
        level: all four migrated modules import nothing ai_os_kernel
        imports at all."""
        violations = scan_pack_source(_SE_PACK_SRC, own_pack_package=_SE_PACK_PACKAGE)
        migrated_modules = {
