...
class _FakeAuditLogWriter:
    """No Postgres needed here — the real, hash-chained audit proof
    already lives in ``tests/integration/git_integration/``. This file
    proves the *dispatch path* (Tool -> adapter -> GitIntegrationService
    -> real SandboxExecutor -> real ``git``), not persistence again."""
...
def _git_service(tmp_path: Path, audit_log: _FakeAuditLogWriter) -> GitIntegrationService:
...
async def _git(args: list[str], *, cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
    """The real, resolved ``git`` binary, off the event loop thread —
    this file's own test-driver calls only, never the Tool under test
...
class TestInvokeAgainstTheRealGitService:
    """(`P03-S01-M24-T02`) Proves the real Git Integration Service is
    genuinely reachable from a Tool call through the full stack — an
    agent's ``context.tools.invoke("platform.git.commit", ...)`` would
    take exactly this path. Every git operation below runs a real
    ``git`` subprocess through a real ``LocalSubprocessSandbox``; each
    test independently reads the real repository/remote back (`git
    log`, `git ls-remote`) rather than trusting the Tool's own return
    value alone — proof this is not a shortcut around
    ``GitIntegrationService``."""

    async def test_available_tools_excludes_git_tools_with_no_git_service_injected(self) -> None:
        adapter = _real_adapter()

        assert adapter.available_tools() == (PLATFORM_SANDBOX_RUN_COMMAND_DESCRIPTOR,)

    async def test_available_tools_includes_git_tools_once_a_git_service_is_injected(
        self, tmp_path: Path
    ) -> None:
        adapter = ToolInvokerAdapter(
            LocalSubprocessSandbox(), git_service=_git_service(tmp_path, _FakeAuditLogWriter())
        )

        descriptors = {d.tool_id for d in adapter.available_tools()}

        assert {PLATFORM_GIT_COMMIT, PLATFORM_GIT_CREATE_BRANCH, PLATFORM_GIT_PUSH} <= descriptors

    async def test_a_git_tool_id_with_no_git_service_falls_through_to_unknown_tool(self) -> None:
        adapter = _real_adapter()  # no git_service — unchanged prior behaviour

        with pytest.raises(UnknownToolError):
            await adapter.invoke(PLATFORM_GIT_COMMIT, {"workspace": "."})

    async def test_a_real_commit_reaches_the_real_service_and_a_real_repo(
        self, tmp_path: Path
    ) -> None:
        ...
        adapter = ToolInvokerAdapter(
            LocalSubprocessSandbox(), git_service=_git_service(tmp_path, audit_log)
        )
        ...
        assert log.stdout.startswith(commit_sha)
        assert "real generated work" in log.stdout
        assert any(r["event_type"] == "git.commit.succeeded" for r in audit_log.records)

    async def test_a_real_branch_create_reaches_the_real_service_and_a_real_repo(
        self, tmp_path: Path
    ) -> None:
        workspace = tmp_path / "workspace"
        workspace.mkdir()
        ...
        audit_log = _FakeAuditLogWriter()
        service = _git_service(tmp_path, audit_log)
        await service.commit(workspace=workspace, message="c1", actor_id="a", actor_type="agent")
        adapter = ToolInvokerAdapter(LocalSubprocessSandbox(), git_service=service)
        ...
        assert result.status is ToolStatus.SUCCESS
        assert result.outputs == {"branch": "feature/real-branch", "created": True}

        current = await _git(["branch", "--show-current"], cwd=workspace)
        assert current.stdout.strip() == "feature/real-branch"

    async def test_a_real_push_reaches_the_real_service_and_a_real_separate_remote(
        self, tmp_path: Path
    ) -> None:
        workspace = tmp_path / "workspace"
        ...
        await service.create_branch(
            workspace=workspace, branch_name="feature/push-me", actor_id="a", actor_type="agent"
        )
        adapter = ToolInvokerAdapter(LocalSubprocessSandbox(), git_service=service)
        ...
        assert result.status is ToolStatus.SUCCESS
        assert result.outputs == {"remote": "origin", "branch": "feature/push-me"}

        refs = await _git(["ls-remote", str(remote)])
        assert "refs/heads/feature/push-me" in refs.stdout

    async def test_a_protected_branch_push_is_refused_through_the_full_stack(
        self, tmp_path: Path
    ) -> None:
        workspace = tmp_path / "workspace"
        remote = tmp_path / "remote.git"
        ...
        assert result.status is ToolStatus.FAILURE
        assert result.error is not None
        assert result.error.error_code == "git.protected_branch_refused"

        refs = await _git(["ls-remote", str(remote)])
        assert refs.stdout == ""

    async def test_malformed_git_inputs_raise_before_reaching_the_service(
        self, tmp_path: Path
    ) -> None:
        adapter = ToolInvokerAdapter(
            LocalSubprocessSandbox(), git_service=_git_service(tmp_path, _FakeAuditLogWriter())
        ...
        with pytest.raises(ValueError, match="input_schema"):
            await adapter.invoke(PLATFORM_GIT_COMMIT, {"workspace": "."})  # missing message/actor
