...
def test_the_real_composition_root_wires_a_real_context_manager_with_a_token_budget(
    ...
    # P02-S03-M08-T13: MemoryResolver rides alongside too -- a plain,
    # unconditional SqlMemoryStore read, added right after
    # WorkflowStateResolver, before RuntimeConfigResolver (T11's own
    # entry, which may be absent depending on AIOS_ENV -- see below).
    assert isinstance(context_manager._resolvers[1], MemoryResolver)
    # P02-S03-M08-T11: RuntimeConfigResolver now rides alongside it in
    # the real production composition -- previously wired nowhere.
    assert isinstance(context_manager._resolvers[2], RuntimeConfigResolver)
